  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (type === "subscription" && username) {
        // サブスク登録：ユーザーにStripe情報を紐づけ＋初期クレジット300付与
        ...
        await sql`
          ...
          VALUES (${username}, 300, 1980, ${session.id})
        `;
    ...
    if (event.type === "invoice.paid") {
      ...
      if (invoice.billing_reason === "subscription_cycle") {
        ...
        if (users.length > 0) {
          ...
          await sql`UPDATE users SET credits = credits + 300 WHERE username = ${username}`;
          await sql`
            INSERT INTO purchase_logs (username, credits_added, amount_yen, stripe_session_id)
            VALUES (${username}, 300, ${invoice.amount_paid || 1980}, ${invoice.id})
          `;
        }
      }
    }
