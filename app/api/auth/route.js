...
export async function POST(request) {
  try {
      ...
      if (resendKey) {
        try {
          await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: "Notti「欲しい」メーカー <noreply@notti.jp>",
              to: [email.trim()],
    ...
    if (action === "getProfile") {
      ...
      const user = await sql`SELECT username, email, birthday, threads_account, nozzle_settings FROM users WHERE username = ${username}`;
