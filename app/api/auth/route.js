...
export async function POST(request) {
  try {
    ...
    if (action === "register") {
      ...
      await sql`
        INSERT INTO users (username, password, email, birthday, threads_account, credits)
        VALUES (${username.trim()}, ${password.trim()}, ${email.trim()}, ${birthday}, ${threadsAccount?.trim() || ""}, 0)
      `;
    ...
    if (action === "getProfile") {
      ...
      const user = await sql`SELECT username, email, birthday, threads_account, nozzle_settings, subscription_status, credits, threads_user_id, threads_token_expires_at FROM users WHERE username = ${username}`;
    ...
    // ═══════ ノズル設定保存 ═══════
    if (action === "saveNozzle") {
      const { username, nozzleSettings } = body;
      if (!username) return NextResponse.json({ error: "ユーザー名が必要です" }, { status: 400 });

      await sql`UPDATE users SET nozzle_settings = ${nozzleSettings || null} WHERE username = ${username}`;
      return NextResponse.json({ result: "ok" });
    }
