...
export async function GET(request) {
  ...
  try {
    ...
    const posts = await sql`
      SELECT id, main_post, reply_post, reply_post2, scheduled_at, status, threads_post_id, error_message, created_at
...
export async function POST(request) {
  try {
    const { username, mainPost, replyPost, replyPost2, scheduledAt } = await request.json();
    ...
    const result = await sql`
      INSERT INTO scheduled_posts (username, main_post, reply_post, reply_post2, scheduled_at)
      VALUES (${username}, ${mainPost.trim()}, ${replyPost?.trim() || null}, ${replyPost2?.trim() || null}, ${scheduledAt})
...
export async function PUT(request) {
  try {
    const { id, username, mainPost, replyPost, replyPost2, scheduledAt } = await request.json();
    ...
    const result = await sql`
      ...
      SET main_post = ${mainPost.trim()}, reply_post = ${replyPost?.trim() || null}, reply_post2 = ${replyPost2?.trim() || null}, scheduled_at = ${scheduledAt}
