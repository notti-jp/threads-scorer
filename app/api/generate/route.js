...
export async function POST(request) {
  ...
  try {
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 16000, system: systemBlocks, messages }),
    });
    ...
    if (type === "b1") {
      ...
      if (appeals.length === 0) return NextResponse.json({ error: "素案の抽出に失敗しました。URLで読み込めない場合は、記事のテキストファイル（.txt）をアップロードしてお試しください" }, { status: 502 });
