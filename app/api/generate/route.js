...
export async function POST(request) {
  ...
  try {
    ...
    if (type === "b2") {
      if (lineBreak === "true") {
        instruction = "この記事を分析し、ゴースト投稿を5案生成してください。各案200文字以内、本投稿のみ。[REPLY]タグは出力しないでください。\n\n【改行ルール】文章の意味のまとまり（段落）ごとに改行してください。段落と段落の間は「　」（全角スペース1文字だけの行）を1行だけ入れてください。空行（何もない行）は入れないでください。2行以上の空白は絶対に作らないでください。1つの段落は2〜4行程度。全体で6〜10行にしてください。";
      } else {
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 6000, system: systemBlocks, messages }),
    });
    ...
    if (type === "b1") {
      ...
      if (appeals.length === 0) return NextResponse.json({ error: `素案の抽出に失敗しました（記事${articleText?.length || 0}文字、AI応答${raw?.length || 0}文字: ${raw?.slice(0, 100) || "空"}）` }, { status: 502 });
