...
export async function POST(request) {
  ...
  try {
    ...
    if (type === "b2") {
      prompt = B2_PROMPT;
      prompt += buildZuruiInstruction("normal");
    } else if (type === "b1") {
      prompt = B1_EXTRACT_PROMPT;
      if (previousAppeals) {
        prompt += B1_REEXTRACT_INSTRUCTION + previousAppeals;
      }
    }
    ...
    if (type === "b2") {
      ...
      instruction = "この記事を多角的に分析し、Threads投稿の「核」になれる魅力要素を5つ抽出してください。投稿文は生成しないでください。";
    }
    ...
    if (type === "b1") {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        if (m) {
          ...
          const content = m.match(/魅力要素：([\s\S]+)/)?.[1]?.trim() || m;
          appeals.push({ angle, content, raw: m });
        }
      }
      if (appeals.length === 0) return NextResponse.json({ error: "魅力要素の抽出に失敗しました" }, { status: 502 });
