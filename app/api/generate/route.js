...
export async function POST(request) {
  ...
  try {
    ...
    // ═══════ 無料パートのみ抽出（freeOnly=1 のとき） ═══════
    const freeOnly = formData.get("freeOnly") === "1";
    let freeOnlyInfo = null; // "marker" | "fallback" | null
    if (freeOnly && type === "b1" && articleText) {
      // 「ここから有料」を含む行を探す（表記ゆれ許容：前後の記号・装飾は無視）
      const lines = articleText.split("\n");
      let markerCharIndex = -1;
      let acc = 0;
      for (const line of lines) {
        if (line.includes("ここから有料")) { markerCharIndex = acc; break; }
        acc += line.length + 1; // +1 は改行分
      }
      if (markerCharIndex >= 0) {
        articleText = articleText.slice(0, markerCharIndex).trim();
        freeOnlyInfo = "marker";
      } else {
        // マーカーがなければ冒頭から全体の1/3（文字数基準）
        const cut = Math.floor(articleText.length / 3);
        articleText = articleText.slice(0, cut).trim();
        freeOnlyInfo = "fallback";
      }
    }
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 16000, system: systemBlocks, messages }),
    });
    ...
    if (type === "b1") {
      ...
      return NextResponse.json({ analysis, appeals, freeOnlyInfo });
    } else {
      // B2: 投稿生成
      const posts = [];
      for (let i = 1; i <= 5; i++) {
