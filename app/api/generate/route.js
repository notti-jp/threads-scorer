...
import { B1_PROMPT, B2_PROMPT, B1_HOOK_CANDIDATES_INSTRUCTION, buildStyleInstruction, buildKnowledgePriority } from "../../prompts";
...
export async function POST(request) {
  ...
  try {
    ...
    if (type === "b2") {
      ...
      prompt = useHook === "true" ? B1_PROMPT + B1_HOOK_CANDIDATES_INSTRUCTION : B1_PROMPT;
    }
    prompt += buildStyleInstruction(styleProfileText);
    prompt += buildKnowledgePriority(knowledgePriority);
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      const hooks = raw.match(new RegExp(`\\[HOOKS${i}\\]\\s*([\\s\\S]*?)\\s*\\[\\/HOOKS${i}\\]`))?.[1]?.trim() || "";
      if (main) posts.push({ main, reply: reply || "", hooks });
    }
