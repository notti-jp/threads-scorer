...
export async function GET() {
  ...
  try {
    ...
    if (pending.length === 0) {
      return NextResponse.json({ debug, message: "投稿予定なし" });
    }
    ...
    if (!post.threads_access_token || !post.threads_user_id) {
      return NextResponse.json({ debug, error: "トークンまたはuser_idが空" });
    }
    ...
    if (!createRes.ok) {
      ...
      return NextResponse.json({ debug, error: "コンテナ作成失敗" });
    }

    const containerIdMatch = createText.match(/"id"\s*:\s*"(\d+)"/);
    if (!containerIdMatch) {
      debug.push("IDパターン不一致。レスポンス全文: " + createText);
      return NextResponse.json({ debug, error: "コンテナID取得失敗" });
    }
    ...
    if (!publishRes.ok) {
      ...
      return NextResponse.json({ debug, error: "公開失敗" });
    }
    ...
    return NextResponse.json({ debug, success: true, threadId });
  } catch (e) {
    debug.push(`エラー: ${e.message}`);
    return NextResponse.json({ debug, error: e.message }, { status: 500 });
  }
}
