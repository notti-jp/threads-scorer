...
import { SCORE_PROMPT, TAB_A_FINAL_CHECK, REPAIR_PROMPT, HOOK_PATTERN_CATALOG, HOOK_CANDIDATES_INSTRUCTION, APPEAL_TO_POST_INSTRUCTION, buildStyleInstruction, buildKnowledgePriorityA, buildZuruiInstructionA } from "../../prompts";
import { getDb } from "../../lib/db";

// ═══════ 使用モデル（タブAのみ。B1/B2・文体学習は claude-sonnet-4-6 のまま） ═══════
// Sonnet 4.6 → Sonnet 5 へ移行。同じ文章のトークン数は約30%増えるが、
// 単価が $3/$15 → $2/$10 に下がるため、差し引きで約13%安くなる。
//
// ★thinking の明示が必須。Sonnet 5 は thinking を省略すると「思考」が自動でONになり、
//   思考トークンが出力と同じ単価で課金される（Sonnet 4.6 は省略＝思考なしだった）。
//   モデル名だけ変えると値下げ分を食い潰すため、明示的に無効化する。
// ★effort は既定値も high だが、既定が変わっても挙動が変わらないよう明示しておく。
...
function codeCheck(main, reply, mode, reply2, originalText) {
  ...
  // ★本投稿＋リプで本投稿が3行を超えても指摘しない（運用判断）。
  //   プロンプト側では引き続き2〜3行を推奨しているが、超えたことを理由に修正パスを走らせない
  ...
  // 22文字ルールを適用するのは「本投稿の冒頭3行」だけ（運用判断）。
  // 以前は投稿全体（本投稿＋リプ）を対象に22文字超×2行で弾いていたが、
  // 日本語は文中に句読点が無い行が普通にあり、自動修正でも折り返せず再生成に回りやすかった。
  // リプ側の読みやすさはプロンプトの推奨と自動修正に任せ、チェックでは強制しない。
  const openingLines = main.split("\n").map(l => l.trim()).filter(l => l && l !== "　").slice(0, 3);
  const longOpening = openingLines.find(l => l.length > 22);
  if (longOpening) {
    issues.push(`本投稿の冒頭が長い（${longOpening.length}文字）。冒頭3行は1文20文字前後で言い切り、テンポで読ませること。該当：「${longOpening.slice(0, 30)}」`);
  }
...
${target}`;
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: MODEL,
        max_tokens: 1000, // 新トークナイザで同じ文章のトークン数が約30%増えるため余裕を持たせる
        thinking: THINKING,
...
function autoRepair(text) {
  ...
  // 長すぎる行を、意味の切れ目（読点・句点）で折り返す。
  // 適切な区切りが無い行は触らない（変な位置で切ると読みにくさが増すため）。
  // 再生成1回＝約18円なので、機械的に直せるものはここで直しきる。
  t = t.split("\n").flatMap(line => {
    const l = line.trim();
    if (l === "　" || l.length <= 22) return [line];
    const out = [];
    let rest = l;
    while (rest.length > 22) {
      // 5〜22文字の範囲で、いちばん後ろにある区切り位置を探す
      // （「去年の11月、」のように短い位置に読点が来ることも多いので下限は低くする）
      let cut = -1;
      for (let i = 5; i <= Math.min(22, rest.length - 1); i++) {
        if ("、。！？".includes(rest[i - 1])) cut = i;
      }
      if (cut === -1) break; // 区切りが無ければ諦めて元のまま残す
  ...
  // 1行だけの段落が中間で連続していたら、あいだの「　」を外してひとまとまりにする。
  // 語句は一切変えず、区切り行を取り除くだけなので安全に自動化できる
  {
    const blocks = [];
    let cur = [];
    ...
    if (cur.length) blocks.push(cur);
    if (blocks.length >= 4) {
      // 前向きに、隣り合う1行段落どうしをまとめる（最終段落は締めなので巻き込まない）
      for (let i = 1; i < blocks.length - 2; i++) {
        if (blocks[i].length === 1 && blocks[i + 1].length === 1) {
          blocks[i] = [...blocks[i], ...blocks[i + 1]];
          blocks.splice(i + 1, 1);
          i--;
        }
      }
      // まとめ切れず残った中間の1行段落を、直前のまとまりに寄せる
      // （直前が3行以内のときだけ。4行を超える段落は作らない）
      for (let i = blocks.length - 2; i >= 2; i--) {
        if (blocks[i].length === 1 && blocks[i - 1].length <= 3) {
          blocks[i - 1] = [...blocks[i - 1], ...blocks[i]];
          blocks.splice(i, 1);
        }
      }
    }
    t = blocks.flatMap((b, i) => (i === 0 ? b : ["　", ...b])).join("\n");
  }
...
export async function POST(request) {
  ...
  try {
      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          if (successes.length > 0) {
            ...
            dynamicPrompt += "→ ★ここに出てくる話し言葉（口調・語尾・言い回し）は、本人が実際に使ったものなので、そのまま使ってよい。逆にここに無い口調を作り出さないこと。\n";
    ...
    // キャッシュTTLは1時間。既定の5分だと、投稿を読んで直している間に必ず切れる。
    // 実測（26リクエスト）では、直前から5分未満の14件は全てHIT、5分以上の11件は全てMISSだった。
    // 1時間にすると書き込みが1.25倍→2倍課金になるが、書き込み回数が12回→5回に減るぶん差し引きで安くなる。
    const systemBlocks = [
    ...
    const generateOnce = async (retryIssues) => {
      ...
      if (!res.ok) {
        // 失敗の内訳をログに残す。従来はレスポンスに返すだけで記録が無く、
        // 429（レート制限）／529（過負荷）／400（リクエスト不正）の区別がつかなかった
        const errBody = await res.text().catch(() => "");
        console.error(`[apierr] generate status=${res.status} body=${errBody.slice(0, 300)}`);
    ...
    // どのチェックで落ちたかを記録する。実測で92%が再生成に回っていたが、
    // 「チェックが厳しすぎる」のか「プロンプトが守らせられていない」のかを
    // 判別する材料が無かった。指摘の冒頭だけを残す（本文は長いので切り詰める）
    if (issues.length > 0) {
      console.log(`[check] ng=${issues.length} ${issues.map(i => i.slice(0, 22)).join(" | ")}`);
    } else {
      console.log("[check] ok");
    }

    // ═══════ 修正パス（軽量） ═══════
    // 従来はフル再生成（約23,000字のプロンプト＝約18円）を走らせていたが、
    // 直すのは既存本文の一部なので、ナレッジ全文も採点指示も要らない。
    // 修正用の小さいプロンプトで本文だけ直し、採点とずるい一言は初回の結果を引き継ぐ。
    const repairConstraints = {
      single: "本投稿のみ。8行以内。[REPLY]は出力しない",
      singleShort: "本投稿のみ。5行以内。[REPLY]は出力しない",
      tree: "本投稿は2〜3行。リプは10行以内",
      treeAuto: "本投稿は2〜3行。リプが13行以上になるなら[REPLY]（12行以内）と[REPLY2]に分割する",
    };

    const repairOnce = async (p, issueList) => {
      let body = `[MAIN]\n${p.main}\n[/MAIN]`;
      if (p.reply) body += `\n[REPLY]\n${p.reply}\n[/REPLY]`;
      if (p.reply2) body += `\n[REPLY2]\n${p.reply2}\n[/REPLY2]`;
      const userMessage =
        `【構成の制約】${repairConstraints[mode] || repairConstraints.tree}\n\n` +
        `【直す対象】\n${body}\n\n` +
        `【直す点（すべて解消すること）】\n${issueList.map((i, n) => `${n + 1}. ${i}`).join("\n")}`;
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01" },
        body: JSON.stringify({
          ...
          thinking: THINKING,
          output_config: OUTPUT_CONFIG,
          system: [{ type: "text", text: REPAIR_PROMPT }],
          messages: [{ role: "user", content: userMessage }],
        }),
      });
      if (!res.ok) throw new Error(`repair error: ${res.status}`);
      const data = await res.json();
      logUsage("repair", data.usage);
      return data.content?.map((c) => c.text || "").join("") || "";
    };

    if (issues.length > 0) {
      try {
        // 本文そのものが出ていない場合だけはフル再生成が要る。それ以外は軽量な修正パス
        const retryRaw = parsed.main ? await repairOnce(parsed, issues) : await generateOnce(issues);
