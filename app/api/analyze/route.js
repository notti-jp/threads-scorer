...
function codeCheck(main, reply, mode, reply2, originalLines) {
  const issues = [];
  const whole = `${main}\n${reply || ""}\n${reply2 || ""}`;
  ...
  if (mode === "treeAuto" && originalLines >= 15 && !(reply2 || "").trim()) issues.push(`元の投稿が${originalLines}行（15行以上）なのにリプが1つしかない。必ず[REPLY]と[REPLY2]の2つに分割して出力すること`);
  if (mode === "treeAuto" && originalLines < 15 && (reply2 || "").trim()) issues.push(`元の投稿が${originalLines}行（15行未満）なのにリプが2つある。[REPLY]1つだけにまとめること`);
...
export async function POST(request) {
  ...
  try {
    ...
    const modeInstructions = {
      ...
      treeAuto: `以下のThreads投稿を採点してください。リライトは「本投稿＋リプ・元の長さに合わせる」モードで出力してください。まず「この投稿全体で伝える気づき」を1行で内部的に決めてから、本投稿=問い／リプ=答えと着地、という1つの話の流れとして書くこと（ツリー全体の一貫性設計に従う）。本投稿は2〜3行（1行だけにせず、フックとして成立する厚みを残す）。★リプの長さは行数を固定せず、入力された元の投稿文の情報量に合わせること：元が短ければリプも短く（無理に引き伸ばさない）、元が長く内容が濃ければリプも長く書く。このモードでは300字上限は適用されない。リプの1行目は前置きを入れず、答えそのもの／リストの1行目／読者の心の声のどれかで即座に書き出すこと（2行目以降は通常どおり展開）。リプは必ず2〜3行ごとに段落を分け、段落と段落の間に「　」（全角スペース1文字だけの行）を入れて読みやすくすること。この区切り行は行数に数えない`,
    };
    // treeAuto: 元の投稿が15行以上なら、リプを2つに分割する指示を追加
    let splitInstruction = "";
    if (mode === "treeAuto") {
      const originalLines = text.split("\n").filter(l => l.trim() !== "" && l !== "　").length;
      if (originalLines >= 15) {
        splitInstruction = `\n\n★★元の投稿は${originalLines}行あります（15行以上）。このためリプは必ず[REPLY]と[REPLY2]の2つに分割して出力すること。[REPLY]に前半（12行程度まで）、[REPLY2]に後半を入れる。意味の区切りのよいところで分けること。[REPLY]（リプ①）の最後は言い切って完結させず、リプ②を自然に読みたくなる状態で終えること（「続きは↓」のような浅い誘導は禁止。内容の力で続きを読ませる。例：核心の直前で止めてリプ②冒頭で即答する／リストの途中で切り「残りが一番効く」と示す）。[REPLY2]の1行目は前置きなしで自然に続きから始める。`;
      } else {
        splitInstruction = `\n\n★元の投稿は${originalLines}行です（15行未満）。リプは[REPLY]1つだけで出力し、[REPLY2]は出力しないこと。`;
      }
    }
    const baseUserMessage = `${modeInstructions[mode] || modeInstructions.tree}${splitInstruction}:\n\n${text}`;
    ...
    const origLinesForCheck = text.split("\n").filter(l => l.trim() !== "" && l !== "　").length;
    ...
    if (!parsed.main) {
      ...
      issues = codeCheck(parsed.main, parsed.reply, mode, parsed.reply2, origLinesForCheck);
      ...
      try {
        ...
        if (retryParsed.main) {
          ...
          if (!parsed.main) {
            ...
            const retryIssues = codeCheck(retryParsed.main, retryParsed.reply, mode, retryParsed.reply2, origLinesForCheck);
            const ngOnly = (arr) => arr.filter(i => i.includes("禁止ワード")).length;
            if (ngOnly(retryIssues) <= ngOnly(codeCheck(parsed.main, parsed.reply, mode, parsed.reply2, origLinesForCheck))) {
              raw = retryRaw;
              parsed = merged;
            }
          }
