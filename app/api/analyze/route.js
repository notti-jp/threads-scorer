...
export async function POST(request) {
  ...
  try {
    ...
    // 出力がトークン上限で切れて改善版が生成されなかった場合
    if (!mainMatch && !replyMatch && data.stop_reason === "max_tokens") {
      return NextResponse.json({ error: "改善版の生成が長すぎて途中で止まりました。もう一度お試しください。" }, { status: 502 });
    }
