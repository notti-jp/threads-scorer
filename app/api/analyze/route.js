...
import { SCORE_PROMPT, TAB_A_FINAL_CHECK, HOOK_PATTERN_CATALOG, HOOK_CANDIDATES_INSTRUCTION, APPEAL_TO_POST_INSTRUCTION, buildStyleInstruction, buildKnowledgePriorityA, buildZuruiInstruction } from "../../prompts";
...
function codeCheck(main, reply, mode, reply2, originalText) {
  ...
  // 1行の長さ。ナレッジの目安は20文字だが、20文字ちょうどで弾くと毎回再生成になってしまう。
  // 「行の長さを一切意識していない」と言える状態（25文字超が3行以上）のときだけ指摘する
  const longLines = whole.split("\n").map(l => l.trim()).filter(l => l !== "" && l !== "　" && l.length > 25);
  if (longLines.length >= 3) {
    const longest = longLines.reduce((a, b) => (b.length > a.length ? b : a));
    issues.push(`1行が長すぎる行が${longLines.length}行ある（最長${longest.length}文字）。スマホの1行は20文字が目安。意味の切れ目で改行すること。該当例：「${longest.slice(0, 30)}…」`);
  }
...
export async function POST(request) {
  ...
  try {
    ...
    // 選び方の方針：
    //   旧: エンゲージメント率（反応数÷ビュー数）の高い順。
    //       ビューが伸びるほど率は下がるため、「フォロワーに濃く刺さったが広がらなかった投稿」ばかりが
    //       選ばれ、伸びない書き方を自分に再学習させていた。
    //   新: リーチした順（views DESC）。ただし反応が薄い当たり投稿を教材にしないよう、
    //       エンゲージメント率が中央値以上のものに絞る。
    //       母数が少ないうち（6件未満）は中央値が意味を持たないので絞り込みを外す。
    // ★ comments が NULL の行に注意。旧クエリは (likes + comments * 2.0) が NULL になり、
    //   PostgreSQL の DESC は既定で NULLS FIRST のため、コメント数未入力の行が無条件に上位を
    //   占めていた。COALESCE で 0 として扱う。
    if (username) {
      try {
        const sql = getDb();
        const successPosts = await sql`
          WITH stats AS (
            SELECT main_post, views,
                   (likes + COALESCE(comments, 0) * 2.0) / views AS eng
            FROM feedbacks
          ...
          agg AS (
            SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY eng) AS med,
                   COUNT(*) AS n
            FROM stats
          )
          SELECT s.main_post, s.views
          FROM stats s CROSS JOIN agg a
          WHERE a.n < 6 OR s.eng >= a.med
          ORDER BY s.views DESC
          LIMIT 3
        `;
        if (successPosts.length > 0) {
          dynamicPrompt += "\n\n【あなたの過去の伸びた投稿（参考）】\n";
          dynamicPrompt += "以下はこのユーザーの過去の投稿のうち、最も多くの人に届いた（ビュー数が伸びた）ものです。\n";
          dynamicPrompt += "フォロワー外にまで広がった投稿なので、1行目で知らない読者の手を止められた実例として扱うこと。\n";
          dynamicPrompt += "特に「1行目が何をして読者を止めたか」に注目し、その効かせ方を参考にすること。\n";
          dynamicPrompt += "ただし、内容をコピーしたり混ぜたりしないこと。あくまで「書き方」の参考として使うこと。\n\n";
          successPosts.forEach((p, i) => {
            const truncated = (p.main_post || "").slice(0, 200);
            const views = p.views ? `${Number(p.views).toLocaleString("ja-JP")}ビュー` : "";
            dynamicPrompt += `伸びた投稿${i + 1}（${views}）：${truncated}\n\n`;
          });
        }
      } catch {}
    }
