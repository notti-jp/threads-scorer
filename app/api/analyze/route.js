...
export async function POST(request) {
  ...
  try {
    ...
    const modeInstructions = {
      ...
      tree: `以下のThreads投稿を採点してください。リライトは「本投稿＋リプ」モードで出力してください。本投稿は2〜3行（1行だけにせず、フックとして成立する厚みを残す)、リプは原則10行以内に収めること。リプの1行目は前置きを入れず、答えそのもの／リストの1行目／読者の心の声のどれかで即座に書き出すこと（2行目以降は通常どおり展開）。リプは必ず2〜3行ごとに段落を分け、段落と段落の間に「　」（全角スペース1文字だけの行）を入れて読みやすくすること。この区切り行は行数に数えない`,
      treeAuto: `以下のThreads投稿を採点してください。リライトは「本投稿＋リプ・元の長さに合わせる」モードで出力してください。本投稿は2〜3行（1行だけにせず、フックとして成立する厚みを残す）。★リプの長さは行数を固定せず、入力された元の投稿文の情報量に合わせること：元が短ければリプも短く（無理に引き伸ばさない）、元が長く内容が濃ければリプも長くしてよい。★ただしリプ1つが12行を超える場合は、意味の区切りのよいところで2つに分け、1つ目を[REPLY]、2つ目を[REPLY2]タグで出力すること。12行以内に収まる場合は[REPLY2]は出力しないこと。リプの1行目は前置きを入れず、答えそのもの／リストの1行目／読者の心の声のどれかで即座に書き出すこと（2行目以降は通常どおり展開）。リプは必ず2〜3行ごとに段落を分け、段落と段落の間に「　」（全角スペース1文字だけの行）を入れて読みやすくすること。この区切り行は行数に数えない`,
    };
    ...
    if (parsed.main) {
      // コード側チェック（0円）
      let issues = codeCheck(parsed.main, parsed.reply, mode, parsed.reply2);
    ...
    return NextResponse.json({
      ...parsed.scores,
      rewrite: (parsed.main || parsed.reply) ? { main: parsed.main, reply: parsed.reply, reply2: parsed.reply2 } : null,
      hooks: parsed.hooks,
    });
  } catch (e) {
