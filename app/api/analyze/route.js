...
function countMiddleSingleLineParagraphs(text) {
  if (!text) return 0;
  const blocks = [];
  let cur = [];
  ...
  if (cur.length) blocks.push(cur);
  return blocks.slice(1, -1).filter(b => b.length === 1).length;
}
...
function codeCheck(main, reply, mode, reply2, originalText) {
  ...
  for (const [label, part] of [["リプ", reply], ["リプ2", reply2]]) {
    const singles = countMiddleSingleLineParagraphs(part);
    if (singles >= 2) {
      issues.push(`${label}の途中に、1行だけの段落が${singles}つある。つながる文を「　」で1行ずつ切り離さず、意味のまとまり2〜4行を1つの段落にして、その後ろに「　」を置くこと。20文字で折り返した行は同じ段落の続きなので「　」を入れない`);
    }
  }
  ...
  const longOpening = openingLines.find(l => l.length > 25);
  if (longOpening) {
