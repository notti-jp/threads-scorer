...
const MODE_CARDS = [
  ...
  {
    value: "tree", title: "本投稿＋リプ", sub: "リプ充実 10行",
    fig: <div>
      <MiniBar w="75%" />
  ...
  {
    value: "treeShort", title: "本投稿＋リプ", sub: "リプ簡潔 8行以内",
    fig: <div>
      <MiniBar w="75%" />
      <div style={{ marginLeft: 8, paddingLeft: 6, borderLeft: "2px solid #83c2cb", marginTop: 4 }}>
        <MiniBar w="75%" color="#83c2cb" /><MiniBar w="45%" color="#83c2cb" />
      </div>
    </div>
  },
];
...
function TabA({ onSaveDraft, onSchedule, draftEdit, styleProfile, onStyleChange, knowledgePriority, onKnowledgeChange, appealText, onClearAppeal, useCredit, username }) {
  ...
  useEffect(() => { if (result?.rewrite) { setEditMain(cleanText(result.rewrite.main || "")); setEditReply(cleanReplyText(result.rewrite.reply || "")); } }, [result]);
  const run = async () => {
          ...
          {result.rewrite && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ maxWidth: 360, margin: "0 auto", width: "100%", background: "#FFFFFF", border: "2px solid #E5E5E5", borderRadius: 20, padding: "12px 20px 20px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                ...
                {editReply && (
                  <div style={{ marginTop: 10, background: "#FAFAFA", borderRadius: 10, padding: 14, borderLeft: "3px solid #83c2cb" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#83c2cb" }}>└ リプ投稿</div><CopyBtn text={editReply} /></div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <Btn onClick={() => onSchedule(editMain, editReply)} label="予約投稿" bg="#83c2cb" color="#FFFFFF" />
                <Btn onClick={() => onSaveDraft(editMain, editReply)} label="改善版を下書き保存" bg="#F5F5F5" color="#666666" />
                <ObsidianBtn large title="改善版" content={editReply ? `${editMain}\n\n***\n\n## リプ投稿\n\n${editReply}` : editMain} label="Obsidianに保存" />
              </div>
            </div>
          )}
        </div>
...
function ScheduledScreen({ posts, onBack, onDelete, onEdit }) {
  ...
  const saveEdit = async () => {
    ...
    try {
      await onEdit(editingPost.id, editMain.trim(), editReply.trim(), scheduledAt.toISOString());
      setEditingPost(null);
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#888" }}>
                    {p.reply_post?.trim() ? "本投稿＋リプ" : "本投稿のみ"}
                  </div>
                </div>
              </div>
            ...
            return (
              <div key={p.id} style={{ marginBottom: 12, padding: 14, background: "#FAFAFA", border: "1px solid rgba(240,184,62,0.2)", borderRadius: 10, borderLeft: "3px solid #f0b83e" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#888" }}>{p.reply_post?.trim() ? "本投稿＋リプ" : "本投稿のみ"}</div>
                </div>
                <div style={{ fontSize: 13, color: "#333", lineHeight: 1.7, whiteSpace: "pre-wrap", marginBottom: 6 }}>{cleanText(p.main_post?.slice(0, 80))}{p.main_post?.length > 80 ? "…" : ""}</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => startEdit(p)} style={{ fontSize: 12, padding: "5px 12px", background: "#fff", border: "1px solid #0a0a0a", borderRadius: 6, color: "#0a0a0a", cursor: "pointer" }}>編集</button>
...
export default function Home() {
  ...
  const handleScheduleConfirm = async (scheduledAt) => {
    ...
    const res = await fetch("/api/scheduled", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, mainPost: scheduleDialog.mainPost, replyPost: scheduleDialog.reply, scheduledAt }) });
    const data = await res.json(); if (!res.ok) throw new Error(data.error);
  ...
  const handleEditScheduled = async (id, mainPost, replyPost, scheduledAt) => {
    const res = await fetch("/api/scheduled", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, username, mainPost, replyPost, scheduledAt }) });
    const data = await res.json(); if (!res.ok) throw new Error(data.error);
