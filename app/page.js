"use client";
// Version: v2.5-StyleLearning - 2026-04-22 - A/B1/B2/Drafts/Style (Vercel版)
...
function StyleLearningDialog({ onComplete, onClose }) {
  ...
  const learn = async () => {
    ...
    try {
      ...
      const styleData = { profile: data.profile, sourceName: file.name, updatedAt: new Date().toISOString() };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#18181b", borderRadius: 14, padding: 24, width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ fontSize: 13, color: "#71717a", marginBottom: 20, lineHeight: 1.6 }}>note記事をアップロードすると、その文体（口調・リズム・表現の癖）を分析し、以降の投稿生成に反映します。</div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={learn} disabled={loading || !file} style={{ flex: 1, padding: 12, borderRadius: 8, border: "none", background: loading ? "#27272a" : "#8b5cf6", color: "#fff", fontSize: 14, fontWeight: 600, cursor: loading ? "wait" : "pointer", opacity: !file ? 0.4 : 1 }}>{loading ? "分析中..." : "学習する"}</button>
        </div>
      </div>
    </div>
  );
