...
const LoadingIndicator = ({ message = "採点中（しばらくお待ちください）" }) => (
  <div style={{ marginTop: 24, marginBottom: 24, textAlign: "center" }}>
    ...
    <div style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "16px 24px", background: "#14141a", border: "1px solid #27272a", borderRadius: 12 }}>
      <div style={{ display: "flex", gap: 4 }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "#8b5cf6", animation: `blink 1.4s ease-in-out ${i * 0.2}s infinite` }} />
        ))}
      </div>
      <span style={{ fontSize: 14, color: "#a1a1aa", animation: "blink 1.4s ease-in-out infinite" }}>{message}</span>
    </div>
  </div>
);
...
function CopyBtn({ text }) {
  ...
  return <button onClick={copy} style={{ fontSize: 11, padding: "3px 10px", background: copied ? "#22c55e22" : "#1e1e24", border: `1px solid ${copied ? "#22c55e44" : "#3f3f46"}`, borderRadius: 6, color: copied ? "#22c55e" : "#a1a1aa", cursor: "pointer", whiteSpace: "nowrap" }}>{copied ? "コピー済" : "コピー"}</button>;
}
const HooksDisplay = ({ hooks }) => {
  if (!hooks) return null;
  return (
    <div style={{ background: "#1a1a10", border: "1px solid #f59e0b33", borderRadius: 10, padding: 16, marginTop: 12 }}>
...
function StyleCard({ style, onChangeRequest }) {
  ...
  if (!style) {
    return (
      <div style={{ marginBottom: 20, padding: 16, background: "#14141a", border: "1px solid #27272a", borderRadius: 10, textAlign: "center" }}>
        <div style={{ fontSize: 13, color: "#71717a", marginBottom: 10 }}>文体が未設定です。記事を読み込ませると、その言い回しで投稿文を生成します。</div>
        <button onClick={onChangeRequest} style={{ padding: "10px 20px", borderRadius: 8, border: "none", background: "#8b5cf6", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>文体を学習させる</button>
      </div>
    );
  }
  return (
    <div style={{ marginBottom: 20, background: "#14141a", border: "1px solid #8b5cf644", borderRadius: 10, overflow: "hidden" }}>
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 14, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#8b5cf6", marginBottom: 4 }}>学習済みの文体</div>
          <div style={{ fontSize: 12, color: "#52525b" }}>{style.sourceName} ・ {new Date(style.updatedAt).toLocaleDateString("ja-JP")}</div>
        </div>
        <span style={{ fontSize: 12, color: "#52525b" }}>{expanded ? "▲" : "▼"}</span>
      </div>
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          <div style={{ fontSize: 13, color: "#a1a1aa", lineHeight: 1.7, whiteSpace: "pre-wrap", marginBottom: 12, maxHeight: 200, overflowY: "auto" }}>{style.profile}</div>
          <button onClick={onChangeRequest} style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #8b5cf644", background: "transparent", color: "#8b5cf6", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>文体を変更する</button>
        </div>
      )}
    </div>
  );
...
function StyleLearningDialog({ onComplete, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#18181b", borderRadius: 14, padding: 24, width: "100%", maxWidth: 400 }}>
        <div style={{ fontSize: 16, fontWeight: 600, color: "#8b5cf6", marginBottom: 6 }}>文体を学習させる</div>
        <div style={{ fontSize: 13, color: "#71717a", marginBottom: 20, lineHeight: 1.6 }}>noteのURLを入力すると、その記事の文体（口調・リズム・表現の癖）を分析し、以降の投稿生成に反映します。</div>
        <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://note.com/..."
          style={{ width: "100%", boxSizing: "border-box", padding: "14px 16px", background: "#0a0a0a", border: "1px solid #27272a", borderRadius: 10, color: "#e4e4e7", fontSize: 14, outline: "none", fontFamily: "inherit", marginBottom: 16 }} />
        {error && <div style={{ marginBottom: 12, padding: 10, background: "rgba(239,68,68,.1)", borderRadius: 8, color: "#ef4444", fontSize: 13 }}>{error}</div>}
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #27272a", background: "transparent", color: "#71717a", fontSize: 14, cursor: "pointer" }}>キャンセル</button>
          <button onClick={learn} disabled={loading || !url.trim()} style={{ flex: 1, padding: 12, borderRadius: 8, border: "none", background: loading ? "#27272a" : "#8b5cf6", color: "#fff", fontSize: 14, fontWeight: 600, cursor: loading ? "wait" : "pointer", opacity: !url.trim() ? 0.4 : 1 }}>{loading ? "分析中..." : "学習する"}</button>
        </div>
      </div>
    </div>
  );
            ...
            {isOpen && (
              <div style={{ padding: "0 14px 14px" }}>
                <div style={{ background: "#0a0a0a", border: "1px solid #1e1e24", borderRadius: 8, padding: 12, borderLeft: "3px solid #e4e4e7", marginBottom: d.reply ? 8 : 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#e4e4e7" }}>本投稿</div><CopyBtn text={d.mainPost} /></div>
                ...
                {d.reply && (
                  <div style={{ marginLeft: 16, background: "#0a0a0a", border: "1px solid #1e1e24", borderRadius: 8, padding: 12, borderLeft: "3px solid #3b82f6", marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#3b82f6" }}>└ リプ投稿</div><CopyBtn text={d.reply} /></div>
...
function TabA({ onSaveDraft, draftEdit, styleProfile, onStyleChange }) {
  ...
  return (
    <div>
      ...
      {text.trim() && !loading && <Btn onClick={() => onSaveDraft(text, "")} label="下書き保存" bg="#1e1e24" color="#a1a1aa" />}
      <Err msg={error} />
...
function TabB1({ onSaveDraft, styleProfile }) {
  ...
  return (
    <div>
      {styleProfile && <div style={{ fontSize: 12, color: "#8b5cf6", marginBottom: 12, padding: "8px 12px", background: "#8b5cf611", borderRadius: 8 }}>文体「{styleProfile.sourceName}」を適用中</div>}
...
export default function Home() {
  ...
  if (!authed) {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#e4e4e7", fontFamily: "'Helvetica Neue', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#e4e4e7", fontFamily: "'Helvetica Neue', sans-serif", padding: "32px 16px", overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
