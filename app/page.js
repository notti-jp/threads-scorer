...
const KN_AREAS = [
  ...
  { id: "algorithm", label: "アルゴリズム攻略" },
  { id: "recognition", label: "認知・集客" },
  { id: "ghost", label: "ゴースト投稿" },
  { id: "profile", label: "プロフ・差別化" },
  { id: "antiAI", label: "AI臭除去" },
];
const KN_PRESETS = {
  balanced: { label: "バランス型", values: { hook: 5, writing: 5, empathy: 5, desire: 5, algorithm: 5, recognition: 5, ghost: 5, profile: 5, antiAI: 5 } },
  empathy: { label: "共感重視", values: { hook: 5, writing: 5, empathy: 9, desire: 7, algorithm: 3, recognition: 3, ghost: 3, profile: 3, antiAI: 5 } },
  writing: { label: "ノウハウ重視（ライティング強化）", values: { hook: 6, writing: 9, empathy: 5, desire: 5, algorithm: 5, recognition: 3, ghost: 3, profile: 3, antiAI: 7 } },
  hookFocus: { label: "フック重視", values: { hook: 9, writing: 6, empathy: 5, desire: 5, algorithm: 5, recognition: 3, ghost: 3, profile: 3, antiAI: 5 } },
  spread: { label: "認知拡大", values: { hook: 7, writing: 5, empathy: 5, desire: 5, algorithm: 7, recognition: 9, ghost: 5, profile: 7, antiAI: 5 } },
};
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div onClick={() => setExpanded(!expanded)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 8, cursor: "pointer" }}>
        <span style={{ fontSize: 13, color: "#2c425a", fontWeight: 600 }}>ナレッジ調整：{preset === "custom" ? "カスタム" : KN_PRESETS[preset]?.label}</span>
      ...
      {expanded && (
        <div style={{ marginTop: 8, padding: 14, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 8 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
            {Object.entries(KN_PRESETS).map(([key, p]) => (
              <button key={key} onClick={() => selectPreset(key)} style={{
                padding: "6px 12px", borderRadius: 6, border: preset === key ? "1px solid #2c425a" : "1px solid #E5E5E5",
                background: preset === key ? "#2c425a" : "#FFFFFF", color: preset === key ? "#FFFFFF" : "#666666",
          ...
          <style>{`
            ...
            input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 18px; height: 18px; border-radius: 50%; background: #FFFFFF; border: 2px solid #2c425a; cursor: pointer; margin-top: -1px; }
          `}</style>
          {KN_AREAS.map(a => (
            <div key={a.id} style={{ marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                <span style={{ color: "#444444" }}>{a.label}</span>
                <span style={{ color: values[a.id] >= 7 ? "#f07852" : values[a.id] >= 4 ? "#2c425a" : "#999", fontWeight: 600 }}>
...
// ═══════ タブA ═══════
function TabA({ onSaveDraft, draftEdit, styleProfile, onStyleChange, knowledgePriority, onKnowledgeChange }) {
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch("/api/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, mode, useHook, styleProfile: styleProfile?.profile || "", knowledgePriority }) });
...
// ═══════ タブB1 ═══════
function TabB1({ onSaveDraft, styleProfile, knowledgePriority, onKnowledgeChange }) {
  ...
  const run = async () => {
    ...
    try {
      const fd = new FormData(); fd.append("file", file); fd.append("type", "b1"); fd.append("mode", mode); fd.append("useHook", String(useHook)); fd.append("styleProfile", styleProfile?.profile || ""); fd.append("knowledgePriority", JSON.stringify(knowledgePriority));
...
// ═══════ タブB2 ═══════
function TabB2({ onSaveDraft, styleProfile, knowledgePriority, onKnowledgeChange }) {
  const [file, setFile] = useState(null);
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null);
  const [error, setError] = useState("");
  const run = async () => {
    if (!file) return; setLoading(true); setError(""); setResult(null);
    try {
      const fd = new FormData(); fd.append("file", file); fd.append("type", "b2"); fd.append("mode", "single"); fd.append("styleProfile", styleProfile?.profile || ""); fd.append("knowledgePriority", JSON.stringify(knowledgePriority));
      const res = await fetch("/api/generate", { method: "POST", body: fd });
      const data = await res.json(); if (!res.ok) throw new Error(data.error); setResult(data);
    } catch (e) { setError(e.message); } finally { setLoading(false); }
  };
...
function ProfileScreen({ username, onBack }) {
  ...
  return (
    <div>
      ...
      {msg && <div style={{ marginBottom: 12, padding: 10, background: "rgba(37,99,235,.06)", borderRadius: 8, color: "#2c425a", fontSize: 13 }}>{msg}</div>}

      <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", cursor: saving ? "wait" : "pointer", background: saving ? "#E5E5E5" : "#2c425a", color: "#FFFFFF", fontSize: 15, fontWeight: 600 }}>
        {saving ? "保存中..." : "変更を保存する"}
      </button>
    </div>
  );
...
export default function Home() {
  ...
  const [knowledgePriority, setKnowledgePriority] = useState(KN_DEFAULT);
  ...
  const btnSt = (loading) => ({ width: "100%", padding: 14, borderRadius: 10, border: "none", cursor: loading ? "wait" : "pointer", background: loading ? "#E5E5E5" : "#2c425a", color: "#FFFFFF", fontSize: 15, fontWeight: 600 });
  ...
  // STEP2: パスワード＋生年月日＋ThreadsID → 登録完了
  const doStep2 = async () => {
    ...
    try {
      ...
      setAuthMode("login");
      setPwErr(""); setPw("");
      toast.show("アカウントを作成しました。ログインしてください。");
    } catch (e) { setPwErr(e.message); } finally { setAuthLoading(false); }
  };
        ...
        <div style={{ width: "100%", maxWidth: 380, textAlign: "center" }}>
          ...
          {authMode === "login" && (<>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, fontSize: 13 }}>
              <button onClick={() => { setAuthMode("register1"); setPwErr(""); setPw(""); }} style={{ background: "none", border: "none", color: "#2c425a", cursor: "pointer", fontSize: 13, textDecoration: "underline" }}>新規登録</button>
          ...
          {/* ═══ 新規登録 STEP1 ═══ */}
          {authMode === "register1" && (<>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#2c425a", marginBottom: 4 }}>STEP 1 / 2</div>
          ...
          {/* ═══ 新規登録 STEP2 ═══ */}
          {authMode === "register2" && (<>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#2c425a", marginBottom: 4 }}>STEP 2 / 2</div>
            ...
            <button onClick={doStep2} disabled={authLoading} style={btnSt(authLoading)}>{authLoading ? "登録中..." : "アカウントを作成"}</button>
          ...
          {authMode === "resetRequest" && (<>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#2c425a", marginBottom: 4 }}>パスワードの再設定</div>
          ...
          {/* ═══ パスワードリセットメール送信完了 ═══ */}
          {authMode === "resetDone" && (<>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#2c425a", marginBottom: 16 }}>メールを送信しました</div>
            <div style={{ fontSize: 14, color: "#444", lineHeight: 1.8, marginBottom: 20, textAlign: "left" }}>
              ご登録済みの<br/>
              <span style={{ fontWeight: 600, color: "#2c425a" }}>{resetMaskedEmail}</span><br/>
          ...
          {authMode === "resetExecute" && (<>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#2c425a", marginBottom: 16 }}>新しいパスワードを設定</div>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <div style={{ marginBottom: 24, textAlign: "center" }}>
          ...
          <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 16, fontSize: 12 }}>
            ...
            <button onClick={() => setScreen("profile")} style={{ background: "none", border: "none", color: "#2c425a", cursor: "pointer", fontSize: 12, textDecoration: "underline" }}>会員情報</button>
        ...
        {screen === "profile" ? (
          ...
          <>
            ...
            {tab === "A" && <TabA onSaveDraft={handleSaveDraft} draftEdit={draftEdit} styleProfile={styleProfile} onStyleChange={() => setShowStyleDialog(true)} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} />}
            {tab === "B1" && <TabB1 onSaveDraft={handleSaveDraft} styleProfile={styleProfile} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} />}
            {tab === "B2" && <TabB2 onSaveDraft={handleSaveDraft} styleProfile={styleProfile} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} />}
