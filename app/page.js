...
const Card = ({ index, main, reply, hooks, onSave, onSchedule }) => (
  <div style={{ marginBottom: 16 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "#666666" }}>案 {index}</div>
      <div style={{ display: "flex", gap: 6 }}>
        {onSchedule && <button onClick={() => onSchedule(main, reply)} style={{ fontSize: 11, padding: "4px 10px", background: "rgba(131,194,203,0.1)", border: "1px solid #83c2cb", borderRadius: 6, color: "#0a0a0a", cursor: "pointer" }}>予約投稿</button>}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 24, width: "100%", maxWidth: 360 }}>
        ...
        <div style={{ fontSize: 11, color: "#888", marginBottom: 16, lineHeight: 1.6 }}>※ Vercel無料プランのため、投稿は1日1回（毎日午前9時）にまとめて実行されます。指定日時を過ぎた投稿が次の実行タイミングで投稿されます。</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {[{ label: "本投稿", text: result.rewrite.main, ml: 0, c: "#0a0a0a" }, { label: "└ リプ投稿", text: result.rewrite.reply, ml: 16, c: "#83c2cb" }].map((p, i) => p.text && (
                <div key={i} style={{ marginLeft: p.ml, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, borderLeft: `3px solid ${p.c}` }}>
                  ...
                  <div style={{ fontSize: 14, color: "#333333", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{cleanText(p.text)}</div>
                </div>
              ))}
              <Btn onClick={() => onSaveDraft(result.rewrite.main, result.rewrite.reply || "")} label="改善版を下書き保存" bg="#F5F5F5" color="#666666" />
              <Btn onClick={() => onSchedule(result.rewrite.main, result.rewrite.reply || "")} label="予約投稿" bg="rgba(131,194,203,0.15)" color="#0a0a0a" />
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {result.appeals?.map((appeal, i) => (
            <div key={i} onClick={() => setSelectedIndex(i)} style={{
              ...
              <div style={{ fontSize: 14, color: "#1A1A1A", lineHeight: 1.9, whiteSpace: "pre-wrap", background: "#FFFFFF", borderRadius: 8, padding: 12, border: "1px solid #E5E5E5", borderLeft: "3px solid #0a0a0a" }}>{cleanText(appeal.content)}</div>
...
export default function Home() {
  ...
  const handleScheduleRequest = (mainPost, reply) => { if (!mainPost?.trim()) return; setScheduleDialog({ mainPost, reply: reply || "" }); };
