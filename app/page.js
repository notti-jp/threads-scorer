...
// ═══════ タブA ═══════
function TabA({ onSaveDraft, draftEdit, styleProfile, onStyleChange, knowledgePriority, onKnowledgeChange, appealText, onClearAppeal, useCredit }) {
...
// ═══════ タブB1：note魅力抽出 ═══════
function TabB1({ onSelectAppeal, knowledgePriority, onKnowledgeChange, useCredit }) {
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {result.appeals?.map((appeal, i) => (
            <div key={i} onClick={() => setSelectedIndex(i)} style={{
              ...
              {appeal.appeal && <div style={{ fontSize: 12, color: "#888", marginBottom: 8, lineHeight: 1.5 }}>{appeal.appeal}</div>}
...
// ═══════ タブB2 ═══════
function TabB2({ onSaveDraft, styleProfile, knowledgePriority, onKnowledgeChange, useCredit }) {
  const [file, setFile] = useState(null);
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null);
  const [error, setError] = useState("");
...
export default function Home() {
  ...
  const handlePurchase = async (planId) => {
    ...
    try {
      const res = await fetch("/api/stripe/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ planId, username }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {screen === "profile" ? (
          ...
          <>
            ...
            {tab === "A" && <TabA onSaveDraft={handleSaveDraft} draftEdit={draftEdit} styleProfile={styleProfile} onStyleChange={() => setShowStyleDialog(true)} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} appealText={appealText} onClearAppeal={() => setAppealText("")} useCredit={useCredit} />}
            {tab === "B1" && <TabB1 onSelectAppeal={handleSelectAppeal} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} useCredit={useCredit} />}
            {tab === "B2" && <TabB2 onSaveDraft={handleSaveDraft} styleProfile={styleProfile} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} useCredit={useCredit} />}
