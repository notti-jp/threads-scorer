...
function ObsidianBtn({ title, content, label }) {
  const [show, setShow] = useState(false);
  return (<>
    <button onClick={(e) => { e.stopPropagation(); setShow(true); }} style={{ fontSize: 11, padding: "3px 10px", background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 6, color: "#7C3AED", cursor: "pointer", whiteSpace: "nowrap" }}>{label || "Obsidian"}</button>
          ...
          {result.rewrite && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <Btn onClick={() => onSchedule(editMain, editReply)} label="予約投稿" bg="rgba(131,194,203,0.15)" color="#0a0a0a" />
                <ObsidianBtn title="改善版" content={editReply ? `${editMain}\n\n---\n\n## リプ投稿\n\n${editReply}` : editMain} label="Obsidianに保存" />
              </div>
            </div>
          )}
        </div>
