...
const Radio = ({ options, value, onChange }) => (
  <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
    {options.map(o => (
      <label key={o.value} onClick={() => onChange(o.value)} style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: value === o.value ? "#F0F0F0" : "#FAFAFA", border: `1px solid ${value === o.value ? "#CCCCCC" : "#E5E5E5"}`, borderRadius: 8, cursor: "pointer" }}>
...
const Btn = ({ onClick, loading, disabled, label, bg, color: c }) => (
  <button onClick={onClick} disabled={loading || disabled} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", cursor: loading ? "wait" : "pointer", background: loading ? "#E5E5E5" : (bg || "#1A1A1A"), color: c || "#FFFFFF", fontSize: 15, fontWeight: 600, opacity: disabled ? 0.4 : 1, marginBottom: 8 }}>{loading ? "生成中..." : label}</button>
);
...
const LoadingIndicator = ({ message = "採点中（しばらくお待ちください）" }) => (
  <div style={{ marginTop: 24, marginBottom: 24, textAlign: "center" }}>
    ...
    <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 16, padding: "24px 32px", background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 16 }}>
      ...
      <span style={{ fontSize: 14, color: "#888888", animation: "blink 1.4s ease-in-out infinite" }}>{message}</span>
    </div>
  </div>
);
...
const HooksDisplay = ({ hooks }) => {
  ...
  return (
    <div style={{ background: "#FAFAFA", border: "1px solid #CCCCCC33", borderRadius: 10, padding: 16, marginTop: 12 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: "#CCCCCC", marginBottom: 10 }}>ずるい一言の候補</div>
      {hooks.split("\n").filter(l => l.trim()).map((line, i) => <div key={i} style={{ fontSize: 14, color: "#1A1A1A", padding: "8px 0", borderTop: i ? "1px solid #CCCCCC22" : "none" }}>{line}</div>)}
    </div>
  );
};
const Card = ({ index, main, reply, hooks, onSave }) => (
  <div style={{ marginBottom: 16 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "#999999" }}>案 {index}</div>
      {onSave && <button onClick={() => onSave(main, reply)} style={{ fontSize: 11, padding: "4px 10px", background: "#F0F0F0", border: "1px solid #CCCCCC", borderRadius: 6, color: "#888888", cursor: "pointer" }}>下書き保存</button>}
    </div>
    {main && <div style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, borderLeft: "3px solid #2c425a", marginBottom: reply ? 8 : 0 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#1A1A1A" }}>本投稿</div><CopyBtn text={main} /></div><div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{main}</div></div>}
    {reply && <div style={{ marginLeft: 16, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, borderLeft: "3px solid #777777" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#999999" }}>└ リプ投稿</div><CopyBtn text={reply} /></div><div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{reply}</div></div>}
    {hooks && <HooksDisplay hooks={hooks} />}
  </div>
);
const HookToggle = ({ value, onChange }) => (
  <div onClick={() => onChange(!value)} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, padding: "10px 14px", background: "#FAFAFA", border: `1px solid ${value ? "rgba(0,0,0,0.08)" : "#E5E5E5"}`, borderRadius: 8, cursor: "pointer" }}>
    <div style={{ width: 40, height: 22, borderRadius: 11, background: value ? "#2c425a" : "#D0D0D0", position: "relative", transition: "background .2s" }}>
...
function StyleCard({ style, onChangeRequest }) {
  ...
  if (!style) {
    return (
      <div style={{ marginBottom: 20, padding: 16, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, textAlign: "center" }}>
        <div style={{ fontSize: 13, color: "#999999", marginBottom: 10 }}>文体が未設定です。記事を読み込ませると、その言い回しで投稿文を生成します。</div>
        <button onClick={onChangeRequest} style={{ padding: "10px 20px", borderRadius: 8, border: "none", background: "#1A1A1A", color: "#FFFFFF", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>文体を学習させる</button>
      </div>
    );
  }
  return (
    <div style={{ marginBottom: 20, background: "#FAFAFA", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 10, overflow: "hidden" }}>
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 14, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1A1A1A", marginBottom: 4 }}>学習済みの文体</div>
          <div style={{ fontSize: 12, color: "#BBBBBB" }}>{style.sourceName} ・ {new Date(style.updatedAt).toLocaleDateString("ja-JP")}</div>
        </div>
        <span style={{ fontSize: 12, color: "#BBBBBB" }}>{expanded ? "▲" : "▼"}</span>
      </div>
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          <div style={{ fontSize: 13, color: "#888888", lineHeight: 1.7, whiteSpace: "pre-wrap", marginBottom: 12, maxHeight: 200, overflowY: "auto" }}>{style.profile}</div>
          <button onClick={onChangeRequest} style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid rgba(0,0,0,0.06)", background: "transparent", color: "#888888", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>文体を変更する</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 24, width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ fontSize: 13, color: "#999999", marginBottom: 20, lineHeight: 1.6 }}>noteのURLを入力すると、その記事の文体（口調・リズム・表現の癖）を分析し、以降の投稿生成に反映します。</div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#888888", fontSize: 14, cursor: "pointer" }}>キャンセル</button>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 24, width: "100%", maxWidth: 360 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#888888", fontSize: 14, cursor: "pointer" }}>キャンセル</button>
          <button onClick={() => onSave(title)} style={{ flex: 1, padding: 12, borderRadius: 8, border: "none", background: "#1A1A1A", color: "#FFFFFF", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>保存</button>
        </div>
      </div>
    </div>
  );
...
function DraftsScreen({ drafts, onBack, onEdit, onDelete, onMarkPosted, statusFilter }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background: "none", border: "none", color: "#999999", fontSize: 14, cursor: "pointer", marginBottom: 16, padding: 0 }}>← 戻る</button>
      <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "#FAFAFA", borderRadius: 10, padding: 4 }}>
        {[{ id: "draft", label: "下書き" }, { id: "posted", label: "投稿済み" }].map(t => (
          <button key={t.id} onClick={() => setFilter(t.id)} style={{ flex: 1, padding: "10px 8px", borderRadius: 8, border: "none", cursor: "pointer", background: filter === t.id ? "#E5E5E5" : "transparent", color: filter === t.id ? "#F5F5F5" : "#777777", fontSize: 13, fontWeight: 600 }}>{t.label}</button>
        ))}
      </div>
      {filtered.length === 0 && <div style={{ textAlign: "center", color: "#BBBBBB", fontSize: 14, padding: 40 }}>{filter === "draft" ? "下書きはありません" : "投稿履歴はありません"}</div>}
      {filtered.map(d => {
          ...
          <div key={d.id} style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, marginBottom: 12, overflow: "hidden" }}>
            <div onClick={() => setExpandedId(isOpen ? null : d.id)} style={{ padding: 14, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#1A1A1A", marginBottom: 4 }}>{d.title}{d.reply && <span style={{ fontSize: 11, color: "#999999", marginLeft: 6 }}>+リプ</span>}</div>
                  <div style={{ fontSize: 11, color: "#BBBBBB" }}>{new Date(d.updatedAt).toLocaleString("ja-JP", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
                </div>
                <span style={{ fontSize: 12, color: "#BBBBBB", marginLeft: 8 }}>{isOpen ? "▲" : "▼"}</span>
              </div>
              {!isOpen && <div style={{ fontSize: 13, color: "#999999", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{d.mainPost}</div>}
            </div>
            {isOpen && (
              <div style={{ padding: "0 14px 14px" }}>
                <div style={{ background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #2c425a", marginBottom: d.reply ? 8 : 12 }}>
                  ...
                  <div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{d.mainPost}</div>
                </div>
                {d.reply && (
                  <div style={{ marginLeft: 16, background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #777777", marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#999999" }}>└ リプ投稿</div><CopyBtn text={d.reply} /></div>
                    <div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{d.reply}</div>
                  </div>
                )}
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  {confirmId === d.id ? (
                    <div style={{ display: "flex", gap: 6, flex: 1 }}>
                      ...
                      <button onClick={() => setConfirmId(null)} style={{ flex: 1, padding: 10, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#888888", fontSize: 12, cursor: "pointer" }}>やめる</button>
                    </div>
                  ) : (
      ...
      {result && (
        <div style={{ marginTop: 28 }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ marginTop: 10, fontSize: 22, fontWeight: 700 }}>{result.total}<span style={{ fontSize: 14, color: "#999999" }}> / 100</span></div>
            <div style={{ marginTop: 6, fontSize: 14, color: "#888888" }}>{result.verdict}</div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 28, marginTop: 28 }}>
            {Object.entries(result.scores).map(([k, v]) => (
              <div key={k}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 5 }}><span style={{ color: "#888888" }}>{LABELS[k]}</span><span style={{ fontWeight: 600 }}>{v.score}<span style={{ color: "#BBBBBB" }}>/20</span></span></div>
                ...
                <div style={{ fontSize: 12, color: "#999999", marginTop: 4 }}>{v.comment}</div>
              </div>
            ))}
          </div>
          {result.suggestions?.length > 0 && (
            <div style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 16, marginBottom: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#888888", marginBottom: 10 }}>改善ポイント</div>
              {result.suggestions.map((s, i) => <div key={i} style={{ fontSize: 13, color: "#E5E5E5", padding: "6px 0", borderTop: i ? "1px solid #F0F0F0" : "none" }}>{i+1}. {s}</div>)}
            </div>
          )}
          {result.rewrite && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#888888" }}>改善版</div>
              {[{ label: "本投稿", text: result.rewrite.main, ml: 0, c: "#F5F5F5" }, { label: "└ リプ投稿", text: result.rewrite.reply, ml: 16, c: "#777777" }].map((p, i) => p.text && (
                <div key={i} style={{ marginLeft: p.ml, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, borderLeft: `3px solid ${p.c}` }}>
                  ...
                  <div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{p.text}</div>
                </div>
              ))}
...
function TabB1({ onSaveDraft, styleProfile }) {
  ...
  return (
    <div>
      {styleProfile && <div style={{ fontSize: 12, color: "#888888", marginBottom: 12, padding: "8px 12px", background: "rgba(0,0,0,0.02)", borderRadius: 8 }}>文体「{styleProfile.sourceName}」を適用中</div>}
      ...
      {result && (
        <div style={{ marginTop: 24 }}>
          {result.analysis && <div style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 16, marginBottom: 20 }}><div style={{ fontSize: 13, fontWeight: 600, color: "#888888", marginBottom: 10 }}>記事分析</div><div style={{ fontSize: 13, color: "#E5E5E5", lineHeight: 1.8, whiteSpace: "pre-wrap" }}>{result.analysis}</div></div>}
          <div style={{ fontSize: 13, fontWeight: 600, color: "#888888", marginBottom: 12 }}>生成結果（5案）</div>
...
function TabB2({ onSaveDraft, styleProfile }) {
  ...
  return (
    <div>
      {styleProfile && <div style={{ fontSize: 12, color: "#888888", marginBottom: 12, padding: "8px 12px", background: "rgba(0,0,0,0.02)", borderRadius: 8 }}>文体「{styleProfile.sourceName}」を適用中</div>}
      <Upload file={file} onChange={setFile} />
      <div style={{ fontSize: 12, color: "#BBBBBB", marginBottom: 16 }}>※ ゴースト投稿は本投稿のみ（8〜12行）で生成されます</div>
...
function TabD({ username }) {
  ...
  const labelStyle = { fontSize: 12, color: "#999999", marginBottom: 4, display: "block" };
  ...
  return (
    <div>
      <div style={{ fontSize: 12, color: "#BBBBBB", marginBottom: 16 }}>ログイン中: {username}</div>

      {/* 統計カード */}
      {feedbacks.length > 0 && (
        <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
          <div style={{ flex: 1, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#999999", marginBottom: 4 }}>投稿数</div>
            <div style={{ fontSize: 22, fontWeight: 700 }}>{feedbacks.length}</div>
          </div>
          <div style={{ flex: 1, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#999999", marginBottom: 4 }}>平均ビュー</div>
            <div style={{ fontSize: 22, fontWeight: 700 }}>{avgViews.toLocaleString()}</div>
          </div>
          <div style={{ flex: 1, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 14, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#999999", marginBottom: 4 }}>平均スコア</div>
      ...
      {/* 入力フォーム表示ボタン */}
      {!showForm && (
        <button onClick={() => setShowForm(true)} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", cursor: "pointer", background: "#1A1A1A", color: "#FFFFFF", fontSize: 15, fontWeight: 600, marginBottom: 20 }}>
          フィードバックを記録する
        </button>
      )}
      ...
      {showForm && (
        <div style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 12, padding: 16, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setShowForm(false); setError(""); }} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#888888", fontSize: 14, cursor: "pointer" }}>キャンセル</button>
      ...
      {/* ローディング */}
      {loading && <div style={{ textAlign: "center", color: "#BBBBBB", padding: 20 }}>読み込み中...</div>}

      {/* フィードバック一覧 */}
      {!loading && feedbacks.length === 0 && !showForm && (
        <div style={{ textAlign: "center", color: "#BBBBBB", fontSize: 14, padding: 40 }}>まだフィードバックがありません</div>
      )}
  ...
  return (
    <div style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, marginBottom: 12, overflow: "hidden" }}>
      <div onClick={() => setExpanded(!expanded)} style={{ padding: 14, cursor: "pointer" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, color: "#999999", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{fb.main_post}</div>
            <div style={{ fontSize: 11, color: "#BBBBBB", marginTop: 6 }}>{new Date(fb.created_at).toLocaleString("ja-JP", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
          </div>
          <span style={{ fontSize: 12, color: "#BBBBBB", marginLeft: 8 }}>{expanded ? "▲" : "▼"}</span>
        </div>
        {/* ミニ指標 */}
      ...
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #2c425a", marginBottom: fb.reply_post ? 8 : 12 }}>
            ...
            <div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{fb.main_post}</div>
          </div>
          {/* リプ */}
          {fb.reply_post && (
            <div style={{ marginLeft: 16, background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #777777", marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#999999", marginBottom: 6 }}>└ リプ投稿</div>
              <div style={{ fontSize: 14, color: "#E5E5E5", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{fb.reply_post}</div>
            </div>
          )}
          {/* 詳細指標 */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={i} style={{ background: "#FFFFFF", borderRadius: 6, padding: 8, textAlign: "center" }}>
                <div style={{ fontSize: 10, color: "#BBBBBB" }}>{x.label}</div>
                <div style={{ fontSize: 16, fontWeight: 600, color: x.color }}>{x.val}</div>
              </div>
            ))}
          </div>
          {fb.hook_type && <div style={{ fontSize: 12, color: "#999999", marginBottom: 12 }}>フック型: {fb.hook_type}</div>}
          {/* 削除ボタン */}
          {confirmDeleteId === fb.id ? (
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <button onClick={() => setConfirmDeleteId(null)} style={{ flex: 1, padding: 10, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#888888", fontSize: 12, cursor: "pointer" }}>やめる</button>
            </div>
          ) : (
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#1A1A1A", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Hiragino Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
        <div style={{ width: "100%", maxWidth: 360, textAlign: "center" }}>
          <div style={{ fontSize: 12, letterSpacing: 4, color: "#999999", textTransform: "uppercase", marginBottom: 6 }}>Threads Tool</div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: "0 0 24px", background: "linear-gradient(135deg, #1A1A1A, #666666)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Threads投稿エンジン</h1>

          {/* ログイン/新規登録 切り替え */}
          <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "#FAFAFA", borderRadius: 10, padding: 4 }}>
            <button onClick={() => { setAuthMode("login"); setPwErr(""); }} style={{ flex: 1, padding: "10px 8px", borderRadius: 8, border: "none", cursor: "pointer", background: authMode === "login" ? "#E5E5E5" : "transparent", color: authMode === "login" ? "#F5F5F5" : "#777777", fontSize: 13, fontWeight: 600 }}>ログイン</button>
            <button onClick={() => { setAuthMode("register"); setPwErr(""); }} style={{ flex: 1, padding: "10px 8px", borderRadius: 8, border: "none", cursor: "pointer", background: authMode === "register" ? "#E5E5E5" : "transparent", color: authMode === "register" ? "#F5F5F5" : "#777777", fontSize: 13, fontWeight: 600 }}>新規登録</button>
          </div>
          ...
          {authMode === "register" && (
            <div style={{ fontSize: 12, color: "#BBBBBB", marginBottom: 12, lineHeight: 1.5, textAlign: "left" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#1A1A1A", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Hiragino Sans', sans-serif", padding: "32px 16px", overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <div style={{ marginBottom: 24, textAlign: "center" }}>
          <div style={{ fontSize: 12, letterSpacing: 4, color: "#999999", textTransform: "uppercase", marginBottom: 6 }}>Threads Tool</div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, background: "linear-gradient(135deg, #1A1A1A, #666666)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Threads投稿エンジン</h1>
        </div>
        {screen === "drafts" ? (
          ...
          <>
            <div style={{ display: "flex", gap: 4, marginBottom: 16, background: "#FAFAFA", borderRadius: 10, padding: 4 }}>
              {TABS.map(t => <button key={t.id} onClick={() => { setTab(t.id); setDraftEdit(null); }} style={{ flex: 1, padding: "10px 8px", borderRadius: 8, border: "none", cursor: "pointer", background: tab === t.id ? "#E5E5E5" : "transparent", color: tab === t.id ? "#F5F5F5" : "#777777", fontSize: 12, fontWeight: 600 }}>{t.label}</button>)}
            </div>
            <button onClick={() => { setDraftsFilter("draft"); setScreen("drafts"); }} style={{ width: "100%", padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#888888", fontSize: 13, cursor: "pointer", marginBottom: 20 }}>
