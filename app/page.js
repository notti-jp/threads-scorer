...
// ═══════ タブB2 ═══════
function TabB2({ onSaveDraft, onSchedule, styleProfile, onStyleChange, useCredit }) {
  const [file, setFile] = useState(null);
  const [noteUrl, setNoteUrl] = useState("");
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null);
    ...
    <div>
      ...
      {result && (
        <div style={{ marginTop: 24 }}>
          ...
          {result.posts.map((p, i) => <Card key={i} index={i+1} main={p.main} reply="" onSave={(m) => onSaveDraft(m, "")} onSchedule={(m) => onSchedule(m, "")} />)}
        </div>
      )}
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#1A1A1A", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Hiragino Sans', sans-serif", padding: "20px 16px", overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {screen === "profile" ? (
          ...
          <div>
            <button onClick={() => setScreen("main")} style={{ background: "none", border: "none", color: "#666666", fontSize: 14, cursor: "pointer", marginBottom: 16, padding: 0 }}>← 戻る</button>
            <div style={{ fontSize: 18, fontWeight: 600, color: "#1A1A1A", marginBottom: 20 }}>予約投稿</div>
            {scheduledPosts.length === 0 && <div style={{ textAlign: "center", color: "#888", fontSize: 14, padding: 40 }}>予約投稿はありません</div>}
            {scheduledPosts.map(p => (
              <div key={p.id} style={{ marginBottom: 16, padding: 16, background: "#FAFAFA", border: `1px solid ${p.status === "posted" ? "#83c2cb" : p.status === "failed" ? "#DC3545" : "#E5E5E5"}`, borderRadius: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <div style={{ fontSize: 12, color: "#888" }}>{new Date(p.scheduled_at).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}</div>
                  <div style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, fontWeight: 600, background: p.status === "posted" ? "rgba(131,194,203,0.15)" : p.status === "failed" ? "rgba(220,53,69,0.1)" : "rgba(240,184,62,0.15)", color: p.status === "posted" ? "#0a0a0a" : p.status === "failed" ? "#DC3545" : "#856404" }}>
                    {p.status === "posted" ? "投稿済み" : p.status === "failed" ? "失敗" : "予約中"}
                  </div>
                </div>
                <div style={{ fontSize: 14, color: "#1A1A1A", lineHeight: 1.8, whiteSpace: "pre-wrap", marginBottom: 8 }}>{cleanText(p.main_post)}</div>
                {p.reply_post && <div style={{ fontSize: 13, color: "#555", lineHeight: 1.7, whiteSpace: "pre-wrap", marginLeft: 16, paddingLeft: 12, borderLeft: "2px solid #83c2cb", marginBottom: 8 }}>{cleanText(p.reply_post)}</div>}
                {p.error_message && <div style={{ fontSize: 12, color: "#DC3545", marginBottom: 8 }}>{p.error_message}</div>}
                {p.status === "pending" && <button onClick={() => handleDeleteScheduled(p.id)} style={{ fontSize: 12, padding: "6px 14px", background: "#FFFFFF", border: "1px solid #DC3545", borderRadius: 6, color: "#DC3545", cursor: "pointer" }}>取り消す</button>}
              </div>
            ))}
          </div>
        ) : (
          <>
            ...
            {tab === "B2" && <TabB2 onSaveDraft={handleSaveDraft} onSchedule={handleScheduleRequest} styleProfile={styleProfile} onStyleChange={() => setShowStyleDialog(true)} useCredit={useCredit} />}
