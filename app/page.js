...
// ═══════ タブA ═══════
function TabA({ onSaveDraft, draftEdit, styleProfile, onStyleChange, knowledgePriority, onKnowledgeChange, appealText, onClearAppeal }) {
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch("/api/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, mode, useHook, styleProfile: styleProfile?.profile || "", knowledgePriority, isAppeal }) });
    ...
    <div>
      ...
      {isAppeal && (
        <div style={{ marginBottom: 12, padding: "10px 14px", background: "rgba(240,184,62,0.08)", border: "1px solid rgba(240,184,62,0.3)", borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 13, color: "#0a0a0a", fontWeight: 600 }}>note魅力抽出から受け取った要素</span>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        <textarea value={text} onChange={e => { setText(e.target.value); if (isAppeal) setIsAppeal(true); }} placeholder={isAppeal ? "魅力要素が入力されています。「採点する」を押すとこの要素で投稿文を生成＋採点します。" : "Threadsの投稿文をここに入力..."} rows={6} style={{ width: "100%", boxSizing: "border-box", background: isAppeal ? "rgba(240,184,62,0.04)" : "#FAFAFA", border: `1px solid ${isAppeal ? "rgba(240,184,62,0.3)" : "#E5E5E5"}`, borderRadius: 12, padding: 16, color: "#1A1A1A", fontSize: 15, lineHeight: 1.7, resize: "vertical", outline: "none", fontFamily: "inherit" }} />
      ...
      <Btn onClick={run} loading={loading} disabled={!text.trim()} label={isAppeal ? "この要素で投稿文を生成＋採点する" : "採点する"} bg="#f07852" />
      {text.trim() && !loading && <Btn onClick={() => onSaveDraft(text, "")} label="下書き保存" bg="#F5F5F5" color="#666666" />}
      <Err msg={error} />
      {loading && <LoadingIndicator message={isAppeal ? "魅力要素から投稿文を生成中..." : "採点中（しばらくお待ちください）"} />}
      {result && (
...
// ═══════ タブB1：note魅力抽出 ═══════
function TabB1({ onSelectAppeal, knowledgePriority, onKnowledgeChange }) {
  ...
  const run = async (isReextract = false) => {
    ...
    try {
      ...
      fd.append("knowledgePriority", JSON.stringify(knowledgePriority));
  ...
  return (
    <div>
      <div style={{ fontSize: 12, color: "#888", marginBottom: 16, lineHeight: 1.6 }}>
        記事をアップロードすると、投稿の「核」になる魅力要素を5つ抽出します。気に入った要素を選んで「投稿文作成へ進む」をクリックすると、タブAで投稿文を作成できます。
      </div>
      <KnowledgeNozzle values={knowledgePriority} onChange={onKnowledgeChange} />
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {result.analysis && (
            <details style={{ marginBottom: 20 }}>
              <summary style={{ fontSize: 13, fontWeight: 600, color: "#555", cursor: "pointer", marginBottom: 8 }}>記事の多角分析（クリックで展開）</summary>
              <div style={{ background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 10, padding: 16 }}>
                <div style={{ fontSize: 13, color: "#444", lineHeight: 1.8, whiteSpace: "pre-wrap" }}>{result.analysis}</div>
              </div>
            </details>
          )}

          {/* 魅力要素一覧 */}
          <div style={{ fontSize: 14, fontWeight: 600, color: "#0a0a0a", marginBottom: 12 }}>抽出された魅力要素（5案）</div>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>気に入った要素をタップして「投稿文作成へ進む」をクリックしてください</div>

          {result.appeals?.map((appeal, i) => (
            <div key={i} onClick={() => setSelectedIndex(i)} style={{
              background: selectedIndex === i ? "rgba(44,66,90,0.06)" : "#FAFAFA",
              ...
              <div style={{ fontSize: 14, color: "#333", lineHeight: 1.8 }}>{appeal.content}</div>
            </div>
          ))}
...
function TabB2({ onSaveDraft, styleProfile, knowledgePriority, onKnowledgeChange }) {
  ...
  return (
    <div>
      {styleProfile && <div style={{ fontSize: 12, color: "#0a0a0a", marginBottom: 12, padding: "8px 12px", background: "rgba(131,194,203,0.08)", border: "1px solid rgba(131,194,203,0.2)", borderRadius: 8 }}>文体「{styleProfile.sourceName}」を適用中</div>}
      <KnowledgeNozzle values={knowledgePriority} onChange={onKnowledgeChange} />
...
export default function Home() {
  ...
  const handleSelectAppeal = (content) => { setAppealText(content); setTab("A"); setScreen("main"); toast.show("魅力要素をタブAに送りました"); };
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#1A1A1A", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Hiragino Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
        <div style={{ width: "100%", maxWidth: 380, textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: "0 0 24px", background: "linear-gradient(135deg, #0a0a0a, #83c2cb)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Threads投稿エンジン</h1>
    ...
    <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#1A1A1A", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Hiragino Sans', sans-serif", padding: "32px 16px", overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <div style={{ marginBottom: 24, textAlign: "center" }}>
          <div style={{ fontSize: 12, letterSpacing: 4, color: "#666666", textTransform: "uppercase", marginBottom: 6 }}>Threads Tool</div>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, background: "linear-gradient(135deg, #0a0a0a, #83c2cb)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Threads投稿エンジン</h1>
        ...
        {screen === "profile" ? (
          ...
          <>
            ...
            {tab === "A" && <TabA onSaveDraft={handleSaveDraft} draftEdit={draftEdit} styleProfile={styleProfile} onStyleChange={() => setShowStyleDialog(true)} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} appealText={appealText} onClearAppeal={() => setAppealText("")} />}
            {tab === "B1" && <TabB1 onSelectAppeal={handleSelectAppeal} knowledgePriority={knowledgePriority} onKnowledgeChange={setKnowledgePriority} />}
