...
// タブAのリプ専用：段落の間隔を全角スペース行で保持する
function cleanReplyText(text) {
  if (!text) return "";
  return text
    .replace(/\n[ \t]*\n[ \t]*\n/g, "\n　\n")  // 3行以上の空白→全角スペース1行
    .replace(/\n[ \t]*\n/g, "\n　\n")            // 空行→全角スペース1行（段落の間隔を保つ）
    .replace(/\n　\n　\n/g, "\n　\n")            // 全角スペース連続→1つに
    .trim();
}
...
// ═══════ 投稿形式セレクター（ミニチュア図付きカード） ═══════
const MiniBar = ({ w, color = "#0a0a0a", op = 1 }) => (
  <div style={{ height: 4, width: w, background: color, opacity: op, borderRadius: 2, marginBottom: 4 }} />
);
const MODE_CARDS = [
  {
    value: "single", title: "本投稿のみ", sub: "しっかり8行",
    fig: <div>
      <MiniBar w="90%" /><MiniBar w="70%" /><MiniBar w="85%" /><MiniBar w="55%" />
    </div>
  },
  {
    value: "singleShort", title: "本投稿のみ", sub: "サクッと5行",
    fig: <div>
      <MiniBar w="90%" /><MiniBar w="55%" />
    </div>
  },
  {
    value: "tree", title: "本投稿＋リプ", sub: "リプ10行",
    fig: <div>
      <MiniBar w="75%" />
      <div style={{ marginLeft: 8, paddingLeft: 6, borderLeft: "2px solid #83c2cb", marginTop: 4 }}>
        <MiniBar w="85%" color="#83c2cb" /><MiniBar w="65%" color="#83c2cb" /><MiniBar w="75%" color="#83c2cb" />
      </div>
    </div>
  },
  {
    value: "treeAuto", title: "本投稿＋リプ", sub: "元の長さに合わせる",
    fig: <div>
      <MiniBar w="75%" />
      <div style={{ marginLeft: 8, paddingLeft: 6, borderLeft: "2px solid #83c2cb", marginTop: 4 }}>
        <MiniBar w="85%" color="#83c2cb" /><MiniBar w="60%" color="#83c2cb" />
      </div>
      <div style={{ marginLeft: 8, paddingLeft: 6, borderLeft: "2px solid #83c2cb", marginTop: 3, opacity: .45 }}>
        <MiniBar w="70%" color="#83c2cb" />
      </div>
    </div>
  },
];
const ModeSelector = ({ value, onChange }) => (
  <div style={{ marginBottom: 16 }}>
    <div style={{ fontSize: 12, color: "#555", marginBottom: 8, fontWeight: 600 }}>投稿の形式</div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
      {MODE_CARDS.map(c => {
        const active = value === c.value;
        return (
          <div key={c.value} onClick={() => onChange(c.value)} style={{
            padding: "10px 12px 8px", borderRadius: 10, cursor: "pointer", transition: "all .15s",
            background: active ? "rgba(0,0,0,0.03)" : "#FAFAFA",
            border: `2px solid ${active ? "#0a0a0a" : "#E5E5E5"}`,
            display: "flex", alignItems: "center", gap: 10
          }}>
            <div style={{ width: 52, flexShrink: 0 }}>{c.fig}</div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#0a0a0a", lineHeight: 1.3 }}>{c.title}</div>
              <div style={{ fontSize: 11, color: active ? "#0a0a0a" : "#888", marginTop: 2 }}>{c.sub}</div>
            </div>
          </div>
        );
      })}
...
// ═══════ 段階表示ローディング（タブA用） ═══════
function StepLoadingIndicator({ steps }) {
  const [idx, setIdx] = useState(0);
  useEffect(() => {
    setIdx(0);
    const timers = steps.map((s, i) =>
      i === 0 ? null : setTimeout(() => setIdx(i), s.at)
    ).filter(Boolean);
    return () => timers.forEach(clearTimeout);
  }, []);
  return (
    <div style={{ marginTop: 24, marginBottom: 24, textAlign: "center" }}>
      <style>{`@keyframes blink { 0%,100%{opacity:1} 50%{opacity:.3} } @keyframes fadeIn { from{opacity:0;transform:translateY(4px)} to{opacity:1;transform:translateY(0)} }`}</style>
      <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 16, padding: "24px 32px", background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 16, minWidth: 240 }}>
        <video autoPlay loop muted playsInline style={{ width: 120, height: 120, borderRadius: 12, objectFit: "cover" }}>
          <source src="/loading.mp4" type="video/mp4" />
        </video>
        <span key={idx} style={{ fontSize: 14, color: "#555555", animation: "fadeIn .4s ease, blink 1.4s ease-in-out infinite .4s" }}>{steps[idx].label}</span>
        <div style={{ display: "flex", gap: 5 }}>
          {steps.map((_, i) => (
            <div key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: i <= idx ? "#0a0a0a" : "#DDD", transition: "background .3s" }} />
          ))}
        </div>
      </div>
    </div>
...
function CopyBtn({ text }) {
  ...
  const copy = async () => { try { await navigator.clipboard.writeText(cleanText(text)); setCopied(true); setTimeout(() => setCopied(false), 1500); } catch {} };
...
// ═══════ Obsidian保存 ═══════
function buildObsidianContent(title, content) {
  const date = new Date();
  const dateStr = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
  const fileName = `キニナル_${dateStr}_${title.replace(/[/\\:*?"<>|]/g, "").slice(0, 20)}`;
  const md = `---\ncreated: ${dateStr}\nsource: キニナルメーカー\n---\n\n${cleanText(content)}`;
  return { fileName, md };
}

function SaveMdDialog({ title, content, onClose }) {
  const { fileName, md } = buildObsidianContent(title, content);
  const openObsidian = () => { window.open(`obsidian://new?name=${encodeURIComponent(fileName)}&content=${encodeURIComponent(md)}`, "_self"); onClose(); };
  const downloadMd = () => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 24, width: "100%", maxWidth: 340 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: "#1A1A1A", marginBottom: 16 }}>投稿文を保存</div>
        <button onClick={openObsidian} style={{ width: "100%", padding: 14, borderRadius: 10, border: "1px solid rgba(124,58,237,0.3)", background: "rgba(124,58,237,0.08)", color: "#7C3AED", fontSize: 14, fontWeight: 600, cursor: "pointer", marginBottom: 8, textAlign: "left" }}>
          Obsidianで開く
          <div style={{ fontSize: 11, fontWeight: 400, color: "#888", marginTop: 4 }}>
            Obsidianアプリが必要です（<a href="https://apps.apple.com/app/obsidian-connected-notes/id1557175442" target="_blank" rel="noopener" style={{ color: "#7C3AED" }}>iOS</a> / <a href="https://play.google.com/store/apps/details?id=md.obsidian" target="_blank" rel="noopener" style={{ color: "#7C3AED" }}>Android</a>）
          </div>
        </button>
        <button onClick={downloadMd} style={{ width: "100%", padding: 14, borderRadius: 10, border: "1px solid #E5E5E5", background: "#FFFFFF", color: "#0a0a0a", fontSize: 14, fontWeight: 600, cursor: "pointer", marginBottom: 8, textAlign: "left" }}>
          .mdファイルでダウンロード
          <div style={{ fontSize: 11, fontWeight: 400, color: "#888", marginTop: 4 }}>どなたでもご利用いただけます</div>
        </button>
        <button onClick={onClose} style={{ width: "100%", padding: 12, borderRadius: 10, border: "none", background: "transparent", color: "#888", fontSize: 13, cursor: "pointer" }}>キャンセル</button>
      </div>
    </div>
  );
}

function ObsidianBtn({ title, content, label, large }) {
  const [show, setShow] = useState(false);
  const btnStyle = large
    ? { width: "100%", padding: "12px 20px", borderRadius: 10, border: "1px solid rgba(124,58,237,0.3)", background: "rgba(124,58,237,0.08)", color: "#7C3AED", fontSize: 13, fontWeight: 600, cursor: "pointer" }
    : { fontSize: 11, padding: "3px 10px", background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 6, color: "#7C3AED", cursor: "pointer", whiteSpace: "nowrap" };
  return (<>
    <button onClick={(e) => { e.stopPropagation(); setShow(true); }} style={btnStyle}>{label || "Obsidian"}</button>
    {show && <SaveMdDialog title={title} content={content} onClose={() => setShow(false)} />}
  </>);
}

function ObsidianBtnAll({ title, contents }) {
  const merged = contents.map((c, i) => `## 案${i + 1}\n\n${cleanText(c)}`).join("\n\n***\n\n");
  return <ObsidianBtn title={title} content={merged} label="全案をObsidianに保存" />;
}
...
function Card({ index, main, reply, hooks, onSave, onSchedule }) {
  const [editMain, setEditMain] = useState(cleanText(main || ""));
  const [editReply, setEditReply] = useState(cleanText(reply || ""));
  useEffect(() => { setEditMain(cleanText(main || "")); setEditReply(cleanText(reply || "")); }, [main, reply]);
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "#666666" }}>案 {index}</div>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          <ObsidianBtn title={`案${index}`} content={editReply ? `${editMain}\n\n***\n\nリプ：\n${editReply}` : editMain} />
          {onSchedule && <button onClick={() => onSchedule(editMain, editReply)} style={{ fontSize: 11, padding: "4px 10px", background: "rgba(131,194,203,0.1)", border: "1px solid #83c2cb", borderRadius: 6, color: "#0a0a0a", cursor: "pointer" }}>予約投稿</button>}
          {onSave && <button onClick={() => onSave(editMain, editReply)} style={{ fontSize: 11, padding: "4px 10px", background: "#F0F0F0", border: "1px solid #CCCCCC", borderRadius: 6, color: "#555555", cursor: "pointer" }}>下書き保存</button>}
        </div>
      </div>
      {/* スマホプレビューフレーム */}
      <div style={{ maxWidth: 320, margin: "0 auto", width: "100%", background: "#FFFFFF", border: "2px solid #E5E5E5", borderRadius: 16, padding: "8px 16px 16px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <div style={{ textAlign: "center", fontSize: 9, color: "#CCC", marginBottom: 6, letterSpacing: 1 }}>THREADS PREVIEW</div>
        {editMain && <div style={{ background: "#FAFAFA", borderRadius: 8, padding: 10, borderLeft: "3px solid #0a0a0a", marginBottom: editReply ? 8 : 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#1A1A1A" }}>本投稿</div><CopyBtn text={editMain} /></div>
          <textarea value={editMain} onChange={e => setEditMain(e.target.value)} style={{ width: "100%", boxSizing: "border-box", fontSize: 14, color: "#333333", lineHeight: 1.9, border: "1px solid #E5E5E5", borderRadius: 8, padding: 10, background: "#FFFFFF", resize: "vertical", minHeight: editReply ? 80 : 120, fontFamily: "inherit", outline: "none" }} />
        </div>}
        {editReply && <div style={{ background: "#FAFAFA", borderRadius: 8, padding: 10, borderLeft: "3px solid #83c2cb" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#0a0a0a" }}>└ リプ投稿</div><CopyBtn text={editReply} /></div>
          <textarea value={editReply} onChange={e => setEditReply(e.target.value)} style={{ width: "100%", boxSizing: "border-box", fontSize: 14, color: "#333333", lineHeight: 1.9, border: "1px solid #E5E5E5", borderRadius: 8, padding: 10, background: "#FFFFFF", resize: "vertical", minHeight: 250, fontFamily: "inherit", outline: "none" }} />
        </div>}
      </div>
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      {expanded && (
        <div style={{ marginTop: 8, padding: 14, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 8 }}>
          ...
          {/* ヘルプボタン */}
          <button onClick={() => setShowHelp(!showHelp)} style={{ fontSize: 12, color: "#83c2cb", background: "none", border: "none", cursor: "pointer", marginBottom: 12, padding: 0 }}>
            {showHelp ? "" : "各ナレッジのスライダーを「強」にすると…"}
          </button>

          {showHelp && (
            <div style={{ marginBottom: 16, padding: 14, background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, fontSize: 12, color: "#444", lineHeight: 1.8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontWeight: 600, color: "#0a0a0a" }}>各スライダーの効果</span>
                <button onClick={() => setShowHelp(false)} style={{ background: "none", border: "none", color: "#888", cursor: "pointer", fontSize: 12 }}>× 閉じる</button>
              </div>
              <div style={{ marginBottom: 6 }}><span style={{ fontWeight: 600 }}>フック力</span>：投稿は1行目が超重要。強いフックで惹きつける。「断言」「問いかけ」「本音」など、スクロールする指を止める冒頭になる。</div>
              <div style={{ marginBottom: 6 }}><span style={{ fontWeight: 600 }}>ライティング技法</span>：文章のリズム・改行・強調語・短文と長文の使い分けが洗練される。読みやすさが上がる。</div>
              <div style={{ marginBottom: 6 }}><span style={{ fontWeight: 600 }}>共感・感情</span>：「これ、私のことだ」と感じさせる表現が増える。読者の気持ちを代弁するトーンになる。</div>
              <div style={{ marginBottom: 6 }}><span style={{ fontWeight: 600 }}>欲望設計</span>：「知りたい」「欲しい」を引き出す構造になる。答えを全部見せず、情報ギャップを残す書き方になる。</div>
              <div style={{ marginBottom: 6 }}><span style={{ fontWeight: 600 }}>認知・集客</span>：フォロワーを増やすことを意識した投稿になる。プロフィールクリックにつながる設計が強まる。</div>
              <div style={{ marginBottom: 6 }}><span style={{ fontWeight: 600 }}>ゴースト投稿</span>：ゴースト投稿作成は別機能でも作成可能だが、24時間で消える限定感・「コッソリ感」を活かした表現になる。秘密を打ち明けるトーンが出る。</div>
              <div><span style={{ fontWeight: 600 }}>プロフ・差別化</span>：「この人は○○の専門家だ」と伝わる投稿になる。発信テーマとnoteの一貫性を意識した内容になる。</div>
            </div>
          )}
...
function ScheduleDialog({ onSchedule, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    if (scheduledAt <= new Date()) { setError("未来の日時を選択してください"); return; }
    setSubmitting(true); setError("");
    try { await onSchedule(scheduledAt.toISOString()); } catch (e) { setError(e.message); } finally { setSubmitting(false); }
  };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
      <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 24, width: "100%", maxWidth: 360 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: "#1A1A1A", marginBottom: 4 }}>予約投稿</div>
        <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>Threadsに自動投稿する日時を指定</div>
        <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>日付</div>
        <input type="date" value={date} onChange={e => { setDate(e.target.value); setError(""); }} style={{ width: "100%", boxSizing: "border-box", padding: "12px 14px", background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, color: "#1A1A1A", fontSize: 14, outline: "none", marginBottom: 12, fontFamily: "inherit" }} />
        <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>時刻</div>
        <input type="time" value={time} onChange={e => { setTime(e.target.value); setError(""); }} style={{ width: "100%", boxSizing: "border-box", padding: "12px 14px", background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, color: "#1A1A1A", fontSize: 14, outline: "none", marginBottom: 12, fontFamily: "inherit" }} />
        <div style={{ fontSize: 11, color: "#888", marginBottom: 16, lineHeight: 1.6 }}>※ 指定した日時になると、自動でThreadsに投稿されます。</div>
        {error && <div style={{ marginBottom: 12, fontSize: 13, color: "#DC3545" }}>{error}</div>}
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#555555", fontSize: 14, cursor: "pointer" }}>キャンセル</button>
          <button onClick={handleSubmit} disabled={submitting} style={{ flex: 1, padding: 12, borderRadius: 8, border: "none", background: "#0a0a0a", color: "#FFFFFF", fontSize: 14, fontWeight: 600, cursor: submitting ? "wait" : "pointer" }}>{submitting ? "登録中..." : "予約する"}</button>
        </div>
      </div>
    </div>
  );
            ...
            {isOpen && (
              <div style={{ padding: "0 14px 14px" }}>
                <div style={{ background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #0a0a0a", marginBottom: d.reply ? 8 : 12 }}>
                  ...
                  <div style={{ fontSize: 14, color: "#333333", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{cleanText(d.mainPost)}</div>
                </div>
                {d.reply && (
                  <div style={{ marginLeft: 16, background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #83c2cb", marginBottom: 12 }}>
                    ...
                    <div style={{ fontSize: 14, color: "#333333", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{cleanText(d.reply)}</div>
                  </div>
                )}
...
// ═══════ タブA ═══════
function TabA({ onSaveDraft, onSchedule, draftEdit, styleProfile, onStyleChange, knowledgePriority, onKnowledgeChange, appealText, onClearAppeal, useCredit, username }) {
  ...
  useEffect(() => { if (result?.rewrite) { setEditMain(cleanText(result.rewrite.main || "")); setEditReply(cleanReplyText(result.rewrite.reply || "")); setEditReply2(cleanReplyText(result.rewrite.reply2 || "")); } }, [result]);
  const run = async () => {
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        <textarea value={text} onChange={e => { setText(e.target.value); if (isAppeal) setIsAppeal(true); }} placeholder={isAppeal ? "素案が入力されています。「この素案で投稿文を生成＋採点する」を押してください。" : "Threadsの投稿文をここに入力..."} rows={10} style={{ width: "100%", boxSizing: "border-box", background: isAppeal ? "rgba(240,184,62,0.04)" : "#FAFAFA", border: `1px solid ${isAppeal ? "rgba(240,184,62,0.3)" : "#E5E5E5"}`, borderRadius: 12, padding: 16, color: "#1A1A1A", fontSize: 15, lineHeight: 1.7, resize: "vertical", outline: "none", fontFamily: "inherit" }} />
      ...
      <Err msg={error} />
      {loading && <StepLoadingIndicator steps={isAppeal ? [
        { at: 0, label: "素案を読み込み中" },
        { at: 4000, label: "投稿文を作成中" },
        { at: 12000, label: "投稿案を確認中" },
        { at: 20000, label: "仕上げています。あと少しです" },
      ] : [
        { at: 0, label: "投稿を採点中" },
        { at: 5000, label: "改善投稿を考案中" },
        { at: 14000, label: "投稿案を確認中" },
        { at: 22000, label: "仕上げています。あと少しです" },
      ]} />}
      {result && (
        <div style={{ marginTop: 28 }}>
          ...
          {result.rewrite && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#555555" }}>改善版（編集できます）</div>
              {/* スマホプレビューフレーム */}
              <div style={{ maxWidth: 360, margin: "0 auto", width: "100%", background: "#FFFFFF", border: "2px solid #E5E5E5", borderRadius: 20, padding: "12px 20px 20px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                <div style={{ textAlign: "center", fontSize: 10, color: "#BBB", marginBottom: 10, letterSpacing: 1 }}>THREADS PREVIEW</div>
                <div style={{ background: "#FAFAFA", borderRadius: 10, padding: 14, borderLeft: "3px solid #0a0a0a" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#0a0a0a" }}>本投稿</div><CopyBtn text={editMain} /></div>
                  <textarea value={editMain} onChange={e => setEditMain(e.target.value)} style={{ width: "100%", boxSizing: "border-box", fontSize: 14, color: "#333333", lineHeight: 1.9, border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, background: "#FFFFFF", resize: "vertical", minHeight: editReply ? 80 : 300, fontFamily: "inherit", outline: "none" }} />
                </div>
                {editReply && (
                  <div style={{ marginTop: 10, background: "#FAFAFA", borderRadius: 10, padding: 14, borderLeft: "3px solid #83c2cb" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#83c2cb" }}>└ リプ投稿{editReply2 ? "①" : ""}</div><CopyBtn text={editReply} /></div>
                    <textarea value={editReply} onChange={e => setEditReply(e.target.value)} style={{ width: "100%", boxSizing: "border-box", fontSize: 14, color: "#333333", lineHeight: 1.9, border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, background: "#FFFFFF", resize: "vertical", minHeight: 400, fontFamily: "inherit", outline: "none" }} />
                  </div>
                )}
                {editReply2 && (
                  <div style={{ marginTop: 10, background: "#FAFAFA", borderRadius: 10, padding: 14, borderLeft: "3px solid #83c2cb" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><div style={{ fontSize: 11, fontWeight: 600, color: "#83c2cb" }}>└ リプ投稿②</div><CopyBtn text={editReply2} /></div>
                    <textarea value={editReply2} onChange={e => setEditReply2(e.target.value)} style={{ width: "100%", boxSizing: "border-box", fontSize: 14, color: "#333333", lineHeight: 1.9, border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, background: "#FFFFFF", resize: "vertical", minHeight: 300, fontFamily: "inherit", outline: "none" }} />
                  </div>
                )}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <Btn onClick={() => onSchedule(editMain, editReply, editReply2)} label="予約投稿" bg="#83c2cb" color="#FFFFFF" />
                <Btn onClick={() => onSaveDraft(editMain, editReply2 ? `${editReply}\n\n${editReply2}` : editReply)} label="改善版を下書き保存" bg="#F5F5F5" color="#666666" />
                <ObsidianBtn large title="改善版" content={editReply ? `${editMain}\n\n***\n\n## リプ投稿\n\n${editReply}${editReply2 ? `\n\n***\n\n## リプ投稿②\n\n${editReply2}` : ""}` : editMain} label="Obsidianに保存" />
              </div>
            </div>
          )}
        </div>
...
// ═══════ タブB1：note魅力抽出 ═══════
function TabB1({ onSelectAppeal, onSchedule, useCredit, styleProfile, onStyleChange }) {
  ...
  const [editedAppeals, setEditedAppeals] = useState({});
  const [freeOnly, setFreeOnly] = useState(false); // false=全文から作る（デフォルト）, true=無料パートのみ

  const run = async (isReextract = false) => {
    if (!file && !noteUrl.trim()) return;
    setLoading(true); setError(""); setResult(null); setSelectedIndex(null); setEditedAppeals({});
    try {
      await useCredit(isReextract ? "tabB1_reextract" : "tabB1_extract");
      const fd = new FormData();
      if (noteUrl.trim()) { fd.append("noteUrl", noteUrl.trim()); } else { fd.append("file", file); }
      fd.append("type", "b1");
      fd.append("mode", "extract");
      fd.append("freeOnly", freeOnly ? "1" : "0");
      ...
      if (data.appeals) {
        ...
        const edited = {};
        data.appeals.forEach((a, i) => { edited[i] = cleanText(a.content || ""); });
        setEditedAppeals(edited);
      }
    } catch (e) { setError(e.message); } finally { setLoading(false); }
  };

  const getEditedContent = (i) => editedAppeals[i] ?? cleanText(result?.appeals?.[i]?.content || "");

  const handleSelect = (i) => {
    onSelectAppeal(getEditedContent(i));
  };
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 12, color: "#555", marginBottom: 6, fontWeight: 600 }}>noteの共有リンクを貼ってください</div>
        <input type="text" value={noteUrl} onChange={e => { setNoteUrl(e.target.value); if (e.target.value.trim()) setFile(null); setResult(null); setPreviousAppeals([]); setSelectedIndex(null); }} placeholder="https://note.com/..." style={{ width: "100%", boxSizing: "border-box", padding: "12px 14px", background: noteUrl.trim() ? "rgba(131,194,203,0.06)" : "#FAFAFA", border: `1px solid ${noteUrl.trim() ? "#83c2cb" : "#E5E5E5"}`, borderRadius: 10, color: "#1A1A1A", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
      </div>
      <div style={{ fontSize: 11, color: "#888", textAlign: "center", marginBottom: 8 }}>または</div>
      <Upload file={file} onChange={f => { setFile(f); if (f) setNoteUrl(""); setResult(null); setPreviousAppeals([]); setSelectedIndex(null); }} />

      {/* 素案の作り方トグル */}
      <div style={{ margin: "16px 0", padding: 16, background: "#FAFAFA", border: "1px solid #E5E5E5", borderRadius: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#1A1A1A", marginBottom: 12 }}>素案の作り方</div>

        <div onClick={() => setFreeOnly(false)} style={{ display: "flex", gap: 10, padding: 12, borderRadius: 10, cursor: "pointer", marginBottom: 8, background: !freeOnly ? "rgba(131,194,203,0.08)" : "transparent", border: `1.5px solid ${!freeOnly ? "#83c2cb" : "#E5E5E5"}` }}>
          <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${!freeOnly ? "#83c2cb" : "#CCC"}`, flexShrink: 0, marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>{!freeOnly && <div style={{ width: 9, height: 9, borderRadius: "50%", background: "#83c2cb" }} />}</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#1A1A1A" }}>核心は伝えない（全文から作る）</div>
            <div style={{ fontSize: 12, color: "#777", marginTop: 3, lineHeight: 1.6 }}>記事全体から作ります。オチや答えは投稿に出さないので、記事を読みたくなる素案になります。</div>
          </div>
        </div>

        <div onClick={() => setFreeOnly(true)} style={{ display: "flex", gap: 10, padding: 12, borderRadius: 10, cursor: "pointer", background: freeOnly ? "rgba(131,194,203,0.08)" : "transparent", border: `1.5px solid ${freeOnly ? "#83c2cb" : "#E5E5E5"}` }}>
          <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${freeOnly ? "#83c2cb" : "#CCC"}`, flexShrink: 0, marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>{freeOnly && <div style={{ width: 9, height: 9, borderRadius: "50%", background: "#83c2cb" }} />}</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#1A1A1A" }}>有料パートは一切使わない</div>
            <div style={{ fontSize: 12, color: "#777", marginTop: 3, lineHeight: 1.6 }}>無料で読める範囲だけで作ります。有料部分は完全に無視します。</div>
          </div>
        </div>

        {freeOnly && (
          <div style={{ marginTop: 12, padding: 12, background: "rgba(240,184,62,0.08)", border: "1px solid rgba(240,184,62,0.25)", borderRadius: 10, fontSize: 12, color: "#7a5c12", lineHeight: 1.7 }}>
            💡 より正確に無料パートを見分けるには、記事の無料と有料の境目に「ここから有料」という一文を入れてください。入っていない場合は、記事の前半（全体の約1/3）を無料パートとして扱います。
          </div>
        )}
      </div>

      <Btn onClick={() => run(false)} loading={loading} disabled={!file && !noteUrl.trim()} label="魅力を抽出する（5案）" bg="#edbb3f" color="#1A1A1A" />
      <Err msg={error} />
      {loading && <StepLoadingIndicator steps={[
        { at: 0, label: "記事を読み込み中" },
        { at: 5000, label: "記事の核心を分析中" },
        { at: 13000, label: "刺さる切り口を探しています" },
        { at: 22000, label: "魅力を抽出中。あと少しです" },
      ]} />}

      {result && (
        <div style={{ marginTop: 24 }}>
          {result.freeOnlyInfo === "marker" && (
            <div style={{ marginBottom: 16, padding: "10px 14px", background: "rgba(131,194,203,0.1)", border: "1px solid rgba(131,194,203,0.3)", borderRadius: 10, fontSize: 12, color: "#2a7a86", lineHeight: 1.6 }}>
              ✓ 無料パート（「ここから有料」より前）のみを使って抽出しました。
            </div>
          )}
          {result.freeOnlyInfo === "fallback" && (
            <div style={{ marginBottom: 16, padding: "10px 14px", background: "rgba(240,184,62,0.1)", border: "1px solid rgba(240,184,62,0.3)", borderRadius: 10, fontSize: 12, color: "#7a5c12", lineHeight: 1.6 }}>
              ℹ️ 「ここから有料」が見つからなかったため、記事の前半（約1/3）を無料パートとして抽出しました。
            </div>
          )}
          <div style={{ fontSize: 14, fontWeight: 600, color: "#0a0a0a", marginBottom: 12 }}>抽出された投稿の素案（5案）</div>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>素案は編集できます。気に入った素案をタップして「投稿文作成へ進む」をクリックしてください</div>
          ...
          {result.appeals?.map((appeal, i) => (
            <div key={i} onClick={() => setSelectedIndex(i)} style={{
              ...
              <div onClick={e => e.stopPropagation()}>
                <textarea value={getEditedContent(i)} onChange={e => setEditedAppeals(prev => ({ ...prev, [i]: e.target.value }))} style={{ width: "100%", boxSizing: "border-box", fontSize: 14, color: "#1A1A1A", lineHeight: 1.9, background: "#FFFFFF", borderRadius: 8, padding: 12, border: "1px solid #E5E5E5", borderLeft: "3px solid #0a0a0a", resize: "vertical", minHeight: 200, fontFamily: "inherit", outline: "none" }} />
              </div>
              <div onClick={e => e.stopPropagation()} style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
                <CopyBtn text={getEditedContent(i)} />
                <ObsidianBtn title={`素案${i + 1}`} content={getEditedContent(i)} />
                {onSchedule && <button onClick={() => onSchedule(getEditedContent(i), "")} style={{ fontSize: 11, padding: "3px 10px", background: "rgba(131,194,203,0.1)", border: "1px solid #83c2cb", borderRadius: 6, color: "#0a0a0a", cursor: "pointer" }}>予約投稿</button>}
              </div>
            </div>
          ))}
          ...
          {result.appeals?.length > 0 && (
            <div style={{ marginTop: 12, marginBottom: 8 }}>
              <ObsidianBtnAll title="素案5案" contents={result.appeals.map((_, i) => getEditedContent(i))} />
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            ...
            <button onClick={() => { if (selectedIndex !== null) handleSelect(selectedIndex); }} disabled={selectedIndex === null} style={{
...
function TabB2({ onSaveDraft, styleProfile, onStyleChange, useCredit }) {
  ...
  const run = async () => {
    if (!file && !noteUrl.trim()) return; setLoading(true); setError(""); setResult(null);
    try {
      await useCredit("tabB2_ghost");
      const fd = new FormData();
      if (noteUrl.trim()) { fd.append("noteUrl", noteUrl.trim()); } else { fd.append("file", file); }
      fd.append("type", "b2"); fd.append("mode", "single"); fd.append("styleProfile", styleProfile?.profile || ""); fd.append("lineBreak", lineBreak ? "true" : "false");
      const res = await fetch("/api/generate", { method: "POST", body: fd });
      const data = await res.json(); if (!res.ok) throw new Error(data.error); setResult(data);
    } catch (e) { setError(e.message); } finally { setLoading(false); }
  };
  return (
    <div>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 12, color: "#555", marginBottom: 6, fontWeight: 600 }}>noteの共有リンクを貼ってください</div>
        <input type="text" value={noteUrl} onChange={e => { setNoteUrl(e.target.value); if (e.target.value.trim()) setFile(null); setResult(null); }} placeholder="https://note.com/..." style={{ width: "100%", boxSizing: "border-box", padding: "12px 14px", background: noteUrl.trim() ? "rgba(131,194,203,0.06)" : "#FAFAFA", border: `1px solid ${noteUrl.trim() ? "#83c2cb" : "#E5E5E5"}`, borderRadius: 10, color: "#1A1A1A", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
      </div>
      <div style={{ fontSize: 11, color: "#888", textAlign: "center", marginBottom: 8 }}>または</div>
      <Upload file={file} onChange={f => { setFile(f); if (f) setNoteUrl(""); setResult(null); }} />
      ...
      <Btn onClick={run} loading={loading} disabled={!file && !noteUrl.trim()} label="ゴースト投稿を生成（5案）" bg="#83c2cb" color="#1A1A1A" />
      <Err msg={error} />
      {loading && <><LoadingIndicator message="生成中（しばらくお待ちください）" /><div style={{ textAlign: "center", fontSize: 12, color: "#888", marginTop: 8 }}>記事の文量が多ければ多いほど、時間がかかります。しばらくお待ちください。</div></>}
      {result && (
        <div style={{ marginTop: 24 }}>
          ...
          {result.posts.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <ObsidianBtnAll title="ゴースト投稿5案" contents={result.posts.map(p => p.main)} />
            </div>
          )}
        </div>
      )}
      ...
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #0a0a0a", marginBottom: fb.reply_post ? 8 : 12 }}>
            ...
            <div style={{ fontSize: 14, color: "#333333", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{cleanText(fb.main_post)}</div>
          </div>
          {/* リプ */}
          {fb.reply_post && (
            <div style={{ marginLeft: 16, background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: 8, padding: 12, borderLeft: "3px solid #83c2cb", marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#0a0a0a", marginBottom: 6 }}>└ リプ投稿</div>
              <div style={{ fontSize: 14, color: "#333333", lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{cleanText(fb.reply_post)}</div>
            </div>
          )}
          {/* 詳細指標 */}
...
// ═══════ 会員情報変更 ═══════
function ProfileScreen({ username, onBack, onPurchase }) {
  ...
  return (
    <div>
      ...
      {profile?.credits !== null && profile?.credits !== undefined && (
        <div style={{ marginBottom: 24, padding: "16px 20px", background: "linear-gradient(135deg, rgba(131,194,203,0.1), rgba(240,184,62,0.08))", borderRadius: 12, border: "1px solid rgba(131,194,203,0.25)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 2 }}>残り利用回数</div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
                <span style={{ fontSize: 28, fontWeight: 700, color: "#0a0a0a" }}>{profile.credits}</span>
                <span style={{ fontSize: 14, color: "#888" }}>回</span>
              </div>
            </div>
            <button onClick={onPurchase} style={{ fontSize: 13, padding: "10px 18px", background: "#0a0a0a", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 600 }}>クレジットを追加</button>
          </div>
        </div>
      )}
      ...
      <div style={{ borderTop: "1px solid #E5E5E5", marginTop: 32, paddingTop: 24 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: "#1A1A1A", marginBottom: 12 }}>Threads連携（予約投稿用）</div>
        {profile?.threads_user_id ? (
          <div>
            <div style={{ padding: "12px 16px", background: "rgba(131,194,203,0.08)", border: "1px solid rgba(131,194,203,0.2)", borderRadius: 8, marginBottom: 12, fontSize: 13, color: "#0a0a0a" }}>
              ✅ Threadsと連携済み
              {profile.threads_token_expires_at && (
                <span style={{ display: "block", fontSize: 11, color: "#888", marginTop: 4 }}>トークン有効期限：{new Date(profile.threads_token_expires_at).toLocaleDateString("ja-JP")}</span>
              )}
            </div>
            <button onClick={async () => {
              try {
                const res = await fetch("/api/threads/connect", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username }) });
                const data = await res.json(); if (!res.ok) throw new Error(data.error);
                window.location.href = data.url;
              } catch (e) { setErr(e.message); }
            }} style={{ width: "100%", padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "#FFFFFF", color: "#555", fontSize: 13, cursor: "pointer" }}>
              再連携する（トークンを更新）
            </button>
          </div>
        ) : (
          <div>
            <div style={{ fontSize: 12, color: "#888", lineHeight: 1.6, marginBottom: 12 }}>
              予約投稿を使うには、Threadsアカウントとの連携が必要です。連携すると、指定した日時にThreadsへ自動で投稿されます。
            </div>
            <button onClick={async () => {
              try {
                const res = await fetch("/api/threads/connect", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username }) });
            ...
            }} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: "#0a0a0a", color: "#FFFFFF", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
              Threadsと連携する
            </button>
          </div>
        )}
...
// ═══════ 予約投稿カレンダー ═══════
function ScheduledScreen({ posts, onBack, onDelete, onEdit }) {
  ...
  const startEdit = (p) => {
    const jst = new Date(p.scheduled_at);
    setEditingPost(p);
    setEditMain(p.main_post || "");
    setEditReply(p.reply_post || "");
    setEditReply2(p.reply_post2 || "");
    setEditDate(`${jst.getFullYear()}-${String(jst.getMonth() + 1).padStart(2, "0")}-${String(jst.getDate()).padStart(2, "0")}`);
    setEditTime(`${String(jst.getHours()).padStart(2, "0")}:${String(jst.getMinutes()).padStart(2, "0")}`);
    setEditError("");
  };

  const saveEdit = async () => {
    if (!editMain.trim()) { setEditError("投稿文を入力してください"); return; }
    if (!editDate) { setEditError("日付を選択してください"); return; }
    const scheduledAt = new Date(`${editDate}T${editTime}:00+09:00`);
    if (scheduledAt <= new Date()) { setEditError("未来の日時を選択してください"); return; }
    setEditSaving(true); setEditError("");
    try {
      await onEdit(editingPost.id, editMain.trim(), editReply.trim(), scheduledAt.toISOString(), editReply2.trim());
      setEditingPost(null);
  ...
  posts.forEach(p => {
    ...
    const key = `${jst.getFullYear()}-${String(jst.getMonth() + 1).padStart(2, "0")}-${String(jst.getDate()).padStart(2, "0")}`;
    if (!postsByDate[key]) postsByDate[key] = [];
    postsByDate[key].push({ ...p, jstDate: jst });
  });
  ...
  const statusColor = (s) => s === "posted" ? "#83c2cb" : s === "failed" ? "#DC3545" : s === "processing" ? "#f07852" : "#f0b83e";
  const statusLabel = (s) => s === "posted" ? "投稿済み" : s === "failed" ? "失敗" : s === "processing" ? "処理中" : "予約中";
  const weekdays = ["日", "月", "火", "水", "木", "金", "土"];

  return (
    <div>
      <button onClick={onBack} style={{ background: "none", border: "none", color: "#666", fontSize: 14, cursor: "pointer", marginBottom: 16, padding: 0 }}>← 戻る</button>
      <div style={{ fontSize: 18, fontWeight: 600, color: "#1A1A1A", marginBottom: 20 }}>予約投稿</div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 600, color: "#0a0a0a" }}>{year}年{month + 1}月</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", marginBottom: 4 }}>
        {weekdays.map((w, i) => (
          <div key={i} style={{ textAlign: "center", fontSize: 11, color: i === 0 ? "#DC3545" : i === 6 ? "#83c2cb" : "#888", fontWeight: 600, padding: "4px 0" }}>{w}</div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 20 }}>
        {days.map((d, i) => {
          ...
          const hasPending = hasPosts?.some(p => p.status === "pending");
          const hasPosted = hasPosts?.some(p => p.status === "posted");
          const hasFailed = hasPosts?.some(p => p.status === "failed");
          return (
            <div key={i} onClick={() => setSelectedDate(isSelected ? null : key)} style={{
              textAlign: "center", padding: "8px 0", borderRadius: 8, cursor: hasPosts ? "pointer" : "default",
              background: isSelected ? "#0a0a0a" : isToday ? "rgba(240,184,62,0.1)" : "transparent",
              border: isToday && !isSelected ? "1px solid #f0b83e" : "1px solid transparent",
              position: "relative"
            }}>
              <div style={{ fontSize: 13, fontWeight: isToday ? 700 : 400, color: isSelected ? "#fff" : i % 7 === 0 ? "#DC3545" : i % 7 === 6 ? "#83c2cb" : "#333" }}>{d}</div>
              {hasPosts && (
                <div style={{ display: "flex", justifyContent: "center", gap: 2, marginTop: 2 }}>
                  {hasPending && <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#f0b83e" }} />}
                  {hasPosted && <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#83c2cb" }} />}
                  {hasFailed && <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#DC3545" }} />}
                </div>
              )}
            </div>
          );
      ...
      {/* ドットの凡例 */}
      <div style={{ display: "flex", justifyContent: "center", gap: 16, marginBottom: 20, fontSize: 11, color: "#888" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "#f0b83e" }} />予約中</div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "#83c2cb" }} />投稿済み</div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "#DC3545" }} />失敗</div>
      </div>

      {/* 選択日の投稿一覧 */}
      {selectedDate && (
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#0a0a0a", marginBottom: 12 }}>
            {selectedDate.replace(/-/g, "/")}の投稿
          </div>
          {selectedPosts.length === 0 && <div style={{ textAlign: "center", color: "#888", fontSize: 13, padding: 20 }}>この日の予約はありません</div>}
          {selectedPosts.sort((a, b) => a.jstDate - b.jstDate).map(p => (
            <div key={p.id} style={{ marginBottom: 12, padding: 14, background: "#FAFAFA", border: `1px solid ${statusColor(p.status)}22`, borderRadius: 10, borderLeft: `3px solid ${statusColor(p.status)}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#0a0a0a" }}>
                    {p.jstDate.getHours().toString().padStart(2, "0")}:{p.jstDate.getMinutes().toString().padStart(2, "0")}
                  </div>
                  <div style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, background: `${statusColor(p.status)}22`, color: statusColor(p.status), fontWeight: 600 }}>
                    {statusLabel(p.status)}
                  </div>
                  <div style={{ fontSize: 11, color: "#888" }}>
                    {p.reply_post2?.trim() ? "本投稿＋リプ×2" : p.reply_post?.trim() ? "本投稿＋リプ" : "本投稿のみ"}
                  </div>
                </div>
              </div>
              <div style={{ fontSize: 13, color: "#333", lineHeight: 1.7, whiteSpace: "pre-wrap", marginBottom: 6 }}>{cleanText(p.main_post?.slice(0, 80))}{p.main_post?.length > 80 ? "…" : ""}</div>
              {p.reply_post?.trim() && <div style={{ fontSize: 12, color: "#888", marginBottom: 6 }}>└ リプ：{p.reply_post.slice(0, 40)}…</div>}
              {p.error_message && <div style={{ fontSize: 12, color: "#DC3545", marginBottom: 6 }}>{p.error_message}</div>}
              {p.status === "pending" && <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => startEdit(p)} style={{ fontSize: 12, padding: "5px 12px", background: "#fff", border: "1px solid #0a0a0a", borderRadius: 6, color: "#0a0a0a", cursor: "pointer" }}>編集</button>
                <button onClick={() => onDelete(p.id)} style={{ fontSize: 12, padding: "5px 12px", background: "#fff", border: "1px solid #DC3545", borderRadius: 6, color: "#DC3545", cursor: "pointer" }}>取り消す</button>
              </div>}
            </div>
          ))}
        </div>
      ...
      {/* 全予約リスト（予約中のみ） */}
      {!selectedDate && posts.filter(p => p.status === "pending").length > 0 && (
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#0a0a0a", marginBottom: 12 }}>予約中の投稿</div>
          {posts.filter(p => p.status === "pending").sort((a, b) => new Date(a.scheduled_at) - new Date(b.scheduled_at)).map(p => {
            const jst = new Date(p.scheduled_at);
            return (
              <div key={p.id} style={{ marginBottom: 12, padding: 14, background: "#FAFAFA", border: "1px solid rgba(240,184,62,0.2)", borderRadius: 10, borderLeft: "3px solid #f0b83e" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#0a0a0a" }}>
                    {jst.getMonth() + 1}/{jst.getDate()} {jst.getHours().toString().padStart(2, "0")}:{jst.getMinutes().toString().padStart(2, "0")}
                  </div>
                  <div style={{ fontSize: 11, color: "#888" }}>{p.reply_post2?.trim() ? "本投稿＋リプ×2" : p.reply_post?.trim() ? "本投稿＋リプ" : "本投稿のみ"}</div>
                </div>
                <div style={{ fontSize: 13, color: "#333", lineHeight: 1.7, whiteSpace: "pre-wrap", marginBottom: 6 }}>{cleanText(p.main_post?.slice(0, 80))}{p.main_post?.length > 80 ? "…" : ""}</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => startEdit(p)} style={{ fontSize: 12, padding: "5px 12px", background: "#fff", border: "1px solid #0a0a0a", borderRadius: 6, color: "#0a0a0a", cursor: "pointer" }}>編集</button>
      ...
      {editingPost && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9998, padding: 16 }}>
          <div style={{ background: "#FAFAFA", borderRadius: 14, padding: 24, width: "100%", maxWidth: 400, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "#1A1A1A", marginBottom: 16 }}>予約投稿を編集</div>

            <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>本投稿</div>
            <textarea value={editMain} onChange={e => setEditMain(e.target.value)} style={{ width: "100%", boxSizing: "border-box", padding: 12, background: "#fff", border: "1px solid #E5E5E5", borderRadius: 8, fontSize: 14, color: "#1A1A1A", lineHeight: 1.8, resize: "vertical", minHeight: 120, fontFamily: "inherit", outline: "none", marginBottom: 12 }} />

            <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>リプ投稿（任意）</div>
            <textarea value={editReply} onChange={e => setEditReply(e.target.value)} placeholder="リプがない場合は空欄のまま" style={{ width: "100%", boxSizing: "border-box", padding: 12, background: "#fff", border: "1px solid #E5E5E5", borderRadius: 8, fontSize: 14, color: "#1A1A1A", lineHeight: 1.8, resize: "vertical", minHeight: 80, fontFamily: "inherit", outline: "none", marginBottom: 12 }} />

            <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>リプ投稿②（任意）</div>
            <textarea value={editReply2} onChange={e => setEditReply2(e.target.value)} placeholder="2つ目のリプがない場合は空欄のまま" style={{ width: "100%", boxSizing: "border-box", padding: 12, background: "#fff", border: "1px solid #E5E5E5", borderRadius: 8, fontSize: 14, color: "#1A1A1A", lineHeight: 1.8, resize: "vertical", minHeight: 80, fontFamily: "inherit", outline: "none", marginBottom: 12 }} />

            <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>日付</div>
            <input type="date" value={editDate} onChange={e => { setEditDate(e.target.value); setEditError(""); }} style={{ width: "100%", boxSizing: "border-box", padding: 12, background: "#fff", border: "1px solid #E5E5E5", borderRadius: 8, fontSize: 14, color: "#1A1A1A", outline: "none", marginBottom: 12, fontFamily: "inherit" }} />

            <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>時刻</div>
            <input type="time" value={editTime} onChange={e => { setEditTime(e.target.value); setEditError(""); }} style={{ width: "100%", boxSizing: "border-box", padding: 12, background: "#fff", border: "1px solid #E5E5E5", borderRadius: 8, fontSize: 14, color: "#1A1A1A", outline: "none", marginBottom: 16, fontFamily: "inherit" }} />

            {editError && <div style={{ marginBottom: 12, fontSize: 13, color: "#DC3545" }}>{editError}</div>}

            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setEditingPost(null)} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #E5E5E5", background: "transparent", color: "#555", fontSize: 14, cursor: "pointer" }}>キャンセル</button>
              <button onClick={saveEdit} disabled={editSaving} style={{ flex: 1, padding: 12, borderRadius: 8, border: "none", background: "#0a0a0a", color: "#fff", fontSize: 14, fontWeight: 600, cursor: editSaving ? "wait" : "pointer" }}>{editSaving ? "保存中..." : "保存する"}</button>
            </div>
          </div>
        </div>
      )}
...
export default function Home() {
  ...
  // ログイン時にDBからノズル設定＋Threads連携状態を復元
  const loadNozzleFromDB = async (uname) => {
    try {
      ...
      if (data.profile?.threads_user_id) setThreadsConnected(true);
    } catch {}
  };
  ...
  const handleScheduleRequest = (mainPost, reply, reply2) => {
    if (!mainPost?.trim()) return;
    if (!threadsConnected) { toast.show("予約投稿を使うには、会員情報からThreadsと連携してください"); return; }
    setScheduleDialog({ mainPost, reply: reply || "", reply2: reply2 || "" });
  };
  const fetchScheduledPosts = async () => {
    if (!username) return;
    try { const res = await fetch(`/api/scheduled?username=${username}`); const data = await res.json(); if (data.posts) setScheduledPosts(data.posts); } catch {}
  };
  const handleScheduleConfirm = async (scheduledAt) => {
    if (!scheduleDialog) return;
    const res = await fetch("/api/scheduled", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, mainPost: scheduleDialog.mainPost, replyPost: scheduleDialog.reply, replyPost2: scheduleDialog.reply2, scheduledAt }) });
    const data = await res.json(); if (!res.ok) throw new Error(data.error);
    toast.show("予約投稿を登録しました"); setScheduleDialog(null); fetchScheduledPosts();
  };
  const handleDeleteScheduled = async (id) => {
    try { await fetch("/api/scheduled", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, username }) }); fetchScheduledPosts(); toast.show("予約を取り消しました"); } catch {}
  };
  const handleEditScheduled = async (id, mainPost, replyPost, scheduledAt, replyPost2) => {
    const res = await fetch("/api/scheduled", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, username, mainPost, replyPost, replyPost2, scheduledAt }) });
    const data = await res.json(); if (!res.ok) throw new Error(data.error);
    fetchScheduledPosts(); toast.show("予約を更新しました");
  };
  useEffect(() => { if (username && authed) fetchScheduledPosts(); }, [username, authed]);
  ...
  useEffect(() => {
    ...
    if (params.get("threads_connect") === "success") { toast.show("Threadsとの連携が完了しました！"); setThreadsConnected(true); window.history.replaceState({}, "", window.location.pathname); }
    if (params.get("threads_connect") === "error") { toast.show("Threadsとの連携に失敗しました。もう一度お試しください。"); window.history.replaceState({}, "", window.location.pathname); }
  }, []);
        ...
        <div style={{ width: "100%", maxWidth: 380, textAlign: "center" }}>
          ...
          {authMode === "login" && (<>
            ...
            <div style={{ textAlign: "right", marginTop: 8, marginBottom: 12 }}>
              <button onClick={() => { setAuthMode("resetRequest"); setPwErr(""); }} style={{ background: "none", border: "none", color: "#888", cursor: "pointer", fontSize: 12 }}>パスワードを忘れた</button>
            </div>
            <button onClick={() => { setAuthMode("register1"); setPwErr(""); setPw(""); }} style={{ width: "100%", padding: 13, borderRadius: 10, border: "2px solid #0a0a0a", background: "#FFFFFF", color: "#0a0a0a", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>はじめての方はこちら（新規登録）</button>
          </>)}
          ...
          {authMode === "register2" && (<>
            ...
            <input type="text" value={regThreads} onChange={e => setRegThreads(e.target.value)} placeholder="ThreadsアカウントID（例：@notti__jp）" style={inputSt} />
    ...
    <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#1A1A1A", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Hiragino Sans', sans-serif", padding: "20px 16px", overflowX: "hidden", width: "100%", maxWidth: "100vw" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <div style={{ marginBottom: 12, textAlign: "center" }}>
          ...
          {/* クレジット残量（残り10以下のときだけ控えめに表示） */}
          {credits !== null && credits <= 10 && (
            <div style={{ marginTop: 12, padding: "8px 14px", background: credits <= 5 ? "rgba(220,53,69,0.06)" : "rgba(240,184,62,0.06)", borderRadius: 10, border: `1px solid ${credits <= 5 ? "rgba(220,53,69,0.2)" : "rgba(240,184,62,0.2)"}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 12, color: credits <= 5 ? "#DC3545" : "#9a7b1e" }}>残り{credits}回になりました</span>
                <button onClick={() => setShowPurchase(true)} style={{ fontSize: 11, padding: "3px 10px", background: "#0a0a0a", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600 }}>追加する</button>
              </div>
            </div>
          )}
        </div>
        ...
        {screen === "profile" ? (
          <ProfileScreen username={username} onBack={() => setScreen("main")} onPurchase={() => { setScreen("main"); setShowPurchase(true); }} />
        ) : screen === "drafts" ? (
          ...
          <ScheduledScreen posts={scheduledPosts} onBack={() => setScreen("main")} onDelete={handleDeleteScheduled} onEdit={handleEditScheduled} />
        ) : (
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
              <button onClick={() => { setDraftsFilter("draft"); setScreen("drafts"); }} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #83c2cb", background: "rgba(131,194,203,0.06)", color: "#0a0a0a", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
                下書き一覧{draftCount > 0 ? `（${draftCount}件）` : ""}
              </button>
              <button onClick={() => setScreen("scheduled")} style={{ flex: 1, padding: 12, borderRadius: 8, border: "1px solid #f0b83e", background: "rgba(240,184,62,0.06)", color: "#0a0a0a", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
                予約投稿{scheduledPosts.filter(p => p.status === "pending").length > 0 ? `（${scheduledPosts.filter(p => p.status === "pending").length}件）` : ""}
              </button>
            </div>
            {tab === "A" && <TabA onSaveDraft={handleSaveDraft} onSchedule={handleScheduleRequest} draftEdit={draftEdit} styleProfile={styleProfile} onStyleChange={() => setShowStyleDialog(true)} knowledgePriority={knowledgePriority} onKnowledgeChange={handleNozzleChange} appealText={appealText} onClearAppeal={() => setAppealText("")} useCredit={useCredit} username={username} />}
            {tab === "B1" && <TabB1 onSelectAppeal={handleSelectAppeal} onSchedule={handleScheduleRequest} useCredit={useCredit} styleProfile={styleProfile} onStyleChange={() => setShowStyleDialog(true)} />}
      ...
      {scheduleDialog && <ScheduleDialog onSchedule={handleScheduleConfirm} onClose={() => setScheduleDialog(null)} />}
